...
public class Preferences {
  ...
  public static int weatherFontColor(Context context) {
    int color =
        Color.parseColor(
            getPrefs(context)
                .getString(Constants.WEATHER_FONT_COLOR, Constants.DEFAULT_LIGHT_COLOR));
    return color;
  }
  ...
  public static boolean setCustomWeatherLocation(Context context, WeatherLocation weatherLocation) {
    ...
    try {
      ...
      getPrefs(context)
          .edit()
          .putString(Constants.WEATHER_CUSTOM_LOCATION, jsonObject.toString())
          .apply();
      return true;
  ...
  public static void setCachedWeatherInfo(Context context, long timestamp, WeatherInfo info) {
    ...
    if (info != null) {
      ...
      try {
        ...
        jsonObject
            .put(WEATHER_INFO_WIND_SPEED, Double.isNaN(windSpeed) ? "NaN" : windSpeed)
            .put(WEATHER_INFO_WIND_SPEED_UNIT, info.getWindSpeedUnit())
            .put(
                WEATHER_INFO_WIND_SPEED_DIRECTION,
                Double.isNaN(windDirection) ? "NaN" : windDirection);

        JSONArray forecastArray = new JSONArray();
        for (WeatherInfo.DayForecast forecast : info.getForecasts()) {
          JSONObject jsonForecast =
              new JSONObject().put(DAY_FORECAST_CONDITION_CODE, forecast.getConditionCode());
  ...
  public static WeatherInfo getCachedWeatherInfo(Context context) {
    ...
    try {
      ...
      WeatherInfo.Builder weatherInfo =
          new WeatherInfo.Builder(city, temperature, tempUnit)
