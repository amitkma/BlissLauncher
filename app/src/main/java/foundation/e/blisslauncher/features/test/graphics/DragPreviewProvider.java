...
public class DragPreviewProvider {
  ...
  public DragPreviewProvider(View view, Context context) {
    ...
    if (mView instanceof IconTextView) {
      Drawable d = ((IconTextView) mView).getIcon();
  ...
  protected void drawDragView(Canvas destCanvas, float scale) {
    ...
    if (mView instanceof IconTextView) {
      ...
      // TODO: When enabling folder support
  ...
  /**
   * Returns a new bitmap to show when the {@link #mView} is being dragged around. Responsibility
  ...
  public Bitmap createDragBitmap() {
    ...
    return BitmapRenderer.createHardwareBitmap(
        width + blurSizeOutline, height + blurSizeOutline, (c) -> drawDragView(c, 1));
  }

  public final void generateDragOutline(Bitmap preview) {
    if (mOutlineGeneratorCallback != null) {
  ...
  public float getScaleAndPosition(Bitmap preview, int[] outPos) {
    float scale =
        TestActivity.Companion.getLauncher(mView.getContext())
            .getDragLayer()
            .getLocationInDragLayer(mView, outPos);
    outPos[0] =
        Math.round(
            outPos[0] - (preview.getWidth() - scale * mView.getWidth() * mView.getScaleX()) / 2);
  ...
  private class OutlineGeneratorCallback implements Runnable {
    ...
    public void run() {
      ...
      canvas.drawBitmap(preview, -thickInnerBlurOffset[0], -thickInnerBlurOffset[1], paint);
      ...
      canvas.drawBitmap(thickInnerBlur, thickInnerBlurOffset[0], thickInnerBlurOffset[1], paint);
