...
public class DragOptions {
  ...
  /**
   * Specifies a condition that must be met before DragListener#onDragStart() is called. By default,
   * there is no condition and onDragStart() is called immediately following
   * DragController#startDrag().
   *
   * <p>This condition can be overridden, and callbacks are provided for the following cases: - The
   * pre-drag starts, but onDragStart() is deferred (onPreDragStart()). - The pre-drag ends before
   * the condition is met (onPreDragEnd(false)). - The actual drag starts when the condition is met
   * (onPreDragEnd(true)).
   */
  public interface PreDragCondition {
    ...
    /**
     * The pre-drag has ended. This gets called at the same time as onDragStart() if the condition
     * is met, otherwise at the same time as onDragEnd().
     *
     * @param dragStarted Whether the pre-drag ended because the actual drag started. This will be
