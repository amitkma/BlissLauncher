...
public class SpringAnimationBuilder {
  ...
  /**
   * The spring equation is given as x = e^(-beta*t/2) * (a cos(gamma * t) + b sin(gamma * t) v =
   * e^(-beta*t/2) * ((2 * a * gamma + beta * b) * sin(gamma * t) + (a * beta - 2 * b * gamma) *
   * cos(gamma * t)) / 2
   *
   * <p>a = x(0) b = beta * x(0) / (2 * gamma) + v(0) / gamma
   */
  private double beta;
  ...
  public SpringAnimationBuilder computeParams() {
    // Update it with DefaultDisplay
    int singleFrameMs = 16;
    ...
    while (duration < 0
        || Math.abs(exponentialComponent(duration) * cosSinV(duration)) >= mVelocityThreshold) {
      duration += piByG;
    }
  ...
  public <T> ValueAnimator build(T target, FloatProperty<T> property) {
    ...
    animator.addUpdateListener(
        anim -> property.set(target, getInterpolatedValue(anim.getAnimatedFraction())));
