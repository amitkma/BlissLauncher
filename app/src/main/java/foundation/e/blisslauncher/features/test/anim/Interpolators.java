...
public class Interpolators {
  ...
  public static final Interpolator ZOOM_OUT =
      new Interpolator() {
        ...
        /**
         * This interpolator emulates the rate at which the perceived scale of an object changes as
         * its distance from a camera increases. When this interpolator is applied to a scale
  ...
  /**
   * Runs the given interpolator such that the entire progress is set between the given bounds. That
   * is, we set the interpolation to 0 until lowerBound and reach 1 by upperBound.
   */
  public static Interpolator clampToProgress(
      Interpolator interpolator, float lowerBound, float upperBound) {
  ...
  /**
   * Runs the given interpolator such that the interpolated value is mapped to the given range. This
   * is useful, for example, if we only use this interpolator for part of the animation, such as to
   * take over a user-controlled animation when they let go.
   */
  ...
  public static class OvershootParams {
     ...
     * @param overshootPastProgress The progress from 0 to 1 where we overshoot past (should either
     *     be equal to startProgress or endProgress, depending on if we want to overshoot
     ...
     */
    public OvershootParams(
        float startProgress,
        float overshootPastProgress,
        float endProgress,
      ...
      float overshootBy =
          OVERSHOOT_FACTOR * velocityPxPerMs * SINGLE_FRAME_MS / totalDistancePx / 2;
      ...
      Interpolator settle =
          Interpolators.clampToProgress(
              Interpolators.mapToProgress(ACCEL_DEACCEL, 1, (endProgress - start) / (end - start)),
              overshootFraction,
              1);
      interpolator =
          t -> t <= overshootFraction ? overshoot.getInterpolation(t) : settle.getInterpolation(t);
    }
  }
}
