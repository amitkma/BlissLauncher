 ...
 *
 * <p>Things to test when changing the following class. - Home from workspace - from center screen -
 * from other screens - Home from all apps - from center screen - from other screens - Back from all
 * apps - from center screen - from other screens - Launch app from workspace and quit - with back -
 * with home - Launch app from all apps and quit - with back - with home - Go to a screen that's not
 * the default, then all apps, and launch and app, and go back - with back -with home - On
 * workspace, long press power and go back - with back - with home - On all apps, long press power
 * and go back - with back - with home - On workspace, power off - On all apps, power off - Launch
 * an app and turn off the screen while in that app - Go back with home key - Go back with back key
 * TODO: make this not go to workspace - From all apps - From workspace - Enter and exit car mode
 * (becase it causes an extra configuration changed) - From all apps - From the center workspace -
...
public class LauncherStateManager {
  ...
  public static final int ANIM_ALL =
      NON_ATOMIC_COMPONENT | ATOMIC_OVERVIEW_PEEK_COMPONENT | ATOMIC_OVERVIEW_SCALE_COMPONENT;
  ...
  public LauncherStateManager(TestActivity l) {
   ...
   *
   * @param animated false if the state should change immediately without any animation, true
   *     otherwise
  ...
  private void goToState(
      LauncherState state, boolean animated, long delay, final Runnable onCompleteRunnable) {
  ...
  private void goToStateAnimated(
      LauncherState state, LauncherState fromState, Runnable onCompleteRunnable) {
    ...
    AnimatorSet animation =
        createAnimationToNewWorkspaceInternal(state, builder, onCompleteRunnable);
    mUiHandler.post(new StartAnimRunnable(animation));
  }

  /**
   * Prepares for a non-user controlled animation from fromState to toState. Preparations include: -
   * Setting interpolators for various animations included in the state transition. - Setting some
   * start values (e.g. scale) for views that are hidden but about to be shown.
   */
  public void prepareForAtomicAnimation(
      LauncherState fromState, LauncherState toState, AnimatorSetBuilder builder) {
  ...
  public AnimatorSet createAtomicAnimation(
      ...
      @AnimationComponents int atomicComponent,
      long duration) {
  ...
  /**
   * Creates a {@link AnimatorPlaybackController} that can be used for a controlled state
   * transition.
   *
   * @param state the final state for the transition.
   * @param duration intended duration for normal playback. Use higher duration for better accuracy.
   */
  ...
  public AnimatorPlaybackController createAnimationToNewWorkspace(
    ...
    return createAnimationToNewWorkspace(
        state, new AnimatorSetBuilder(), duration, null, animComponents);
  }
  ...
  public AnimatorPlaybackController createAnimationToNewWorkspace(
    ...
    mConfig.playbackController =
        AnimatorPlaybackController.wrap(
            createAnimationToNewWorkspaceInternal(state, builder, null),
            duration,
            onCancelRunnable);
    return mConfig.playbackController;
  }

  protected AnimatorSet createAnimationToNewWorkspaceInternal(
      final LauncherState state, AnimatorSetBuilder builder, final Runnable onCompleteRunnable) {
  ...
  private void onStateTransitionStart(LauncherState state) {
    ...
    if (state.disablePageClipping) {
      ...
      mLauncher.getLauncherPagedView().setClipChildren(false);
    }
  ...
  private void onStateTransitionEnd(LauncherState state) {
    ...
    mLauncher.getLauncherPagedView().setClipChildren(!state.disablePageClipping);
  ...
  /**
   * Sets the animation as the current state animation, i.e., canceled when starting another
   * animation and may block some launcher interactions while running.
   *
  ...
  public interface StateHandler {
    ...
    void setStateWithAnimation(
        LauncherState toState, AnimatorSetBuilder builder, AnimationConfig config);
  }
