...
public class WidgetPicker extends Activity implements WidgetPickerAdapter.OnClickListener {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    if (parcel instanceof Intent) {
      ...
      mBaseIntent.setFlags(
          mBaseIntent.getFlags()
    ...
    Collections.sort(
        items,
        new Comparator<WidgetPickerAdapter.Item>() {
    ...
    if (info.icon != 0) {
      try {
        ...
        Resources packageResources =
            mPackageManager.getResourcesForApplication(info.provider.getPackageName());
        icon = packageResources.getDrawableForDensity(info.icon, iconDensity);
      } catch (PackageManager.NameNotFoundException e) {
        Log.w(
            TAG,
            "Can't load icon drawable 0x"
                + Integer.toHexString(info.icon)
                + " for provider: "
                + info.provider);
      }
      if (icon == null) {
  ...
  public void onClick(WidgetPickerAdapter.Item item) {
    ...
    if (item.extras != null) {
      ...
      try {
        ...
        boolean success =
            mAppWidgetManager.bindAppWidgetIdIfAllowed(
                mAppWidgetId, intent.getComponent(), options);
        if (success) {
          result = RESULT_OK;
        } else {
  ...
  /**
   * Convenience method for setting the result code and intent. This method correctly injects the
   * {@link AppWidgetManager#EXTRA_APPWIDGET_ID} that most hosts expect returned.
   */
  ...
  protected void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (requestCode == REQUEST_BIND_APPWIDGET) {
      int appWidgetId =
          data != null ? data.getIntExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, -1) : -1;
