 ...
 *
 * <p>All of the animations are done in the Folder. ie. When the user taps on the FolderIcon, we
...
public class FolderAnimationManager {
  ...
  public FolderAnimationManager(Folder folder, boolean isOpening) {
    ...
    mFolderInterpolator = AnimationUtils.loadInterpolator(mContext, R.anim.folder_interpolator);
  }
  ...
  public AnimatorSet getAnimator() {
    ...
    float scaleRelativeToDragLayer =
        mLauncher.getDragLayer().getDescendantRectRelativeToSelf(mFolderIcon, folderIconPos);
    // Calculate the starting and ending bounds for the zoomed-in image.
    // This step involves lots of math. Yay, math.
    Rect dragLayerBounds = new Rect();
    Point globalOffset = new Point();

    mLauncher.getDragLayer().getGlobalVisibleRect(dragLayerBounds, globalOffset);
    ...
    play(a, getAnimator(mFolder, View.X, folderIconPos.left, dragLayerBounds.left));
