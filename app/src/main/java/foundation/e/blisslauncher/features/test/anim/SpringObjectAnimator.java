...
public class SpringObjectAnimator<T> extends ValueAnimator {
  ...
  public SpringObjectAnimator(
      T object,
      FloatProperty<T> property,
    ...
    mSpring.setSpring(new SpringForce(0).setDampingRatio(damping).setStiffness(stiffness));
  ...
  private void tryEnding() {
    ...
    if (mAnimatorEnded && (mSpringEnded || !Flags.QUICKSTEP_SPRINGS) && !mEnded) {
      for (AnimatorListener l : mListeners) {
