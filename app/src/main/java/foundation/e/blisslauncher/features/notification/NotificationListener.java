...
/**
 * A {@link NotificationListenerService} that sends updates to its {@link
 * NotificationsChangedListener} when notifications are posted or canceled, as well and when this
 * service first connects. An instance of NotificationListener, and its methods for getting
 * notifications, can be obtained via {@link #getInstanceIfConnected()}.
 */
...
public class NotificationListener extends NotificationListenerService {
            ...
            case MSG_NOTIFICATION_POSTED:
              if (sNotificationsChangedListener != null) {
                ...
                sNotificationsChangedListener.onNotificationPosted(
                    msg.packageUserKey, msg.notificationKey, msg.shouldBeFilteredOut);
              }
              break;
            case MSG_NOTIFICATION_REMOVED:
              if (sNotificationsChangedListener != null) {
  ...
  public void onNotificationPosted(final StatusBarNotification sbn) {
    ...
    mWorkerHandler
        .obtainMessage(MSG_NOTIFICATION_POSTED, new NotificationPostedMsg(sbn))
        .sendToTarget();
  }
  ...
  public void onNotificationRemoved(final StatusBarNotification sbn) {
    ...
    Pair<PackageUserKey, NotificationKeyData> packageUserKeyAndNotificationKey =
        new Pair<>(PackageUserKey.fromNotification(sbn), NotificationKeyData.fromNotification(sbn));
    mWorkerHandler
        .obtainMessage(MSG_NOTIFICATION_REMOVED, packageUserKeyAndNotificationKey)
        .sendToTarget();
  ...
  public List<StatusBarNotification> getNotificationsForKeys(List<NotificationKeyData> keys) {
    StatusBarNotification[] notifications =
        NotificationListener.this.getActiveNotifications(
            NotificationKeyData.extractKeysOnly(keys).toArray(new String[keys.size()]));
  ...
  /**
   * Filter out notifications that don't have an intent or are headers for grouped notifications.
   *
  ...
  private List<StatusBarNotification> filterNotifications(StatusBarNotification[] notifications) {
    ...
    List<StatusBarNotification> filteredNotifications =
        new ArrayList<>(notifications.length - removedNotifications.size());
    for (int i = 0; i < notifications.length; i++) {
  ...
  public interface NotificationsChangedListener {
    ...
    void onNotificationRemoved(
        PackageUserKey removedPackageUserKey, NotificationKeyData notificationKey);
