...
public class DragController implements DragDriver.EventListener, TouchController {
  ...
  public DragController(TestActivity launcher) {
    mLauncher = launcher;
  }

  /**
   * Starts a drag. When the drag is started, the UI automatically goes into spring loaded mode. On
   * a successful drop, it is the responsibility of the {@link DropTarget} to exit out of the spring
  ...
  public DragView startDrag(
    ...
    mIsInPreDrag =
        mOptions.preDragCondition != null && !mOptions.preDragCondition.shouldStartDrag(0);

    final Resources res = mLauncher.getResources();
    final float scaleDps =
        mIsInPreDrag ? res.getDimensionPixelSize(R.dimen.pre_drag_view_scale) : 0f;
  ...
  public void animateDragViewToOriginalPosition(
      final Runnable onComplete, final View originalIcon, int duration) {
  ...
  /**
   * As above, since accessible drag and drop won't cause the same sequence of touch events, we
   * manually ensure appropriate drag and drop events get emulated for accessible drag.
   */
  ...
  private DropTarget findDropTarget(int x, int y, int[] dropCoordinates) {
    ...
    mLauncher
        .getDragLayer()
        .mapCoordInSelfToDescendant(mLauncher.getLauncherPagedView(), dropCoordinates);
