...
public class AdaptiveIconGenerator {
  ...
  private void loop() {
    ...
    // Any icon with less than 10% transparent pixels (padding excluded) is considered
    // "full-bleed-ish"
    ...
    // not yet checked = not set to false = has to be full bleed, isBackgroundWhite = true = is
    // adaptive
  ...
  private Drawable genResult() {
    AdaptiveIconDrawableCompat tmp =
        new AdaptiveIconDrawableCompat(new ColorDrawable(), new FixedScaleDrawable());
