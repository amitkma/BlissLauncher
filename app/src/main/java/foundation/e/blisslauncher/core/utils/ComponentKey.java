...
public class ComponentKey {
   ...
   * [flattenedComponentString#userId]. If the userId is not present, then it defaults to the
   * current user.
   */
  public ComponentKey(Context context, String componentKeyStr) {
    ...
    if (userDelimiterIndex != -1) {
      ...
      user =
          UserManagerCompat.getInstance(context).getUserForSerialNumber(componentUser.longValue());
    } else {
