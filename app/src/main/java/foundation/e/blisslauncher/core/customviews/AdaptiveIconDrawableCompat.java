...
/**
 * This class can also be created via XML inflation using <code>&lt;adaptive-icon></code> tag in
 * addition to dynamic creation.
 *
 ...
 * </ul>
 *
 * Such motion effect is achieved by internally setting the bounds of the foreground and background
 * layer as following:
 *
 * <pre>
...
public class AdaptiveIconDrawableCompat extends Drawable implements Drawable.Callback {
  ...
  /**
   * All four sides of the layers are padded with extra inset so as to provide extra content to
   * reveal within the clip path when performing affine transformations on the layers.
   *
   * <p>Each layers will reserve 25% of it's width and height.
   *
   * <p>As a result, the view port of the layers is smaller than their intrinsic width and height.
   */
  ...
  /**
   * The one constructor to rule them all. This is called by all public constructors to set the
   * state and initialize local properties.
   */
  ...
  private void initReflections() {
    try {
      ...
      methodCreatePathFromPathData =
          pathParser.getDeclaredMethod("createPathFromPathData", String.class);
  ...
  public AdaptiveIconDrawableCompat(
      Drawable backgroundDrawable, Drawable foregroundDrawable, boolean useMyUglyWorkaround) {
    this((LayerState) null, null);
   ...
   * R.string.config_icon_mask. After the bound is set, the returned path's computed bound is same
   * as the #getBounds().
   *
  ...
  /**
   * Returns the foreground drawable managed by this class. The bound of this drawable is extended
   * by {@link #getExtraInsetFraction()} * getBounds().width on left/right sides and by {@link
  ...
  private Drawable getDrawable(TypedArray a, int index) {
    ...
    if (value.resourceId != 0) {
      return a.getResources()
          .getDrawableForDensity(value.resourceId, DisplayMetrics.DENSITY_DEFAULT, null);
    }
    return null;
  }
  ...
  /**
   * Resumes child invalidation after suspension, immediately performing an invalidation if one was
   * requested by a child during suspension.
   *
  ...
  protected static @NonNull TypedArray obtainAttributes(
  ...
  static class ChildDrawable {
    ...
    ChildDrawable(
        @NonNull ChildDrawable orig,
        @NonNull AdaptiveIconDrawableCompat owner,
