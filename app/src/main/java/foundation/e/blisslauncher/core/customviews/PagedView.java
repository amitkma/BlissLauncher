...
import static foundation.e.blisslauncher.core.touch.OverScroll.OVERSCROLL_DAMP_FACTOR;
...
public abstract class PagedView<T extends View & PageIndicator> extends ViewGroup {
  ...
  protected static final ComputePageScrollsLogic SIMPLE_SCROLL_LOGIC =
      (v) -> v.getVisibility() != GONE;

  public static final int PAGE_SNAP_ANIMATION_DURATION = 750;

  // OverScroll constants
  private static final int OVERSCROLL_PAGE_SNAP_ANIMATION_DURATION = 270;
  private static final float RETURN_TO_ORIGINAL_PAGE_THRESHOLD = 0.5f;
   ...
   * long-pressed on to enter the overview). Try using {@link #getPageNearestToCenterOfScreen()} to
   * get the page the user is currently scrolling over.
   */
  ...
  protected void scrollAndForceFinish(int scrollX) {
    scrollTo(scrollX, 0);
    mScroller.startScroll(mScroller.getCurrPos(), scrollX - mScroller.getCurrPos());
    forceFinishScroller(true);
  }

  /**
   * Updates the scroll of the current page immediately to its final scroll position. We use this in
   * CustomizePagedView to allow tabs to share the same PagedView while resetting the scroll of the
   * previous tab page.
   */
  ...
  /**
   * Should be called whenever the page changes. In the case of a scroll, we wait until the page has
   * settled.
   */
  ...
  /**
   * Called when the page starts moving as part of the scroll. Subclasses can override this to
   * provide custom behavior during animation.
   */
  protected void onPageBeginTransition() {}

  /**
   * Called when the page ends moving as part of the scroll. Subclasses can override this to provide
   * custom behavior during animation.
   */
  ...
  protected boolean computeScrollHelper(boolean shouldInvalidate) {
    if (mScroller.computeScrollOffset()) {
      ...
      if (getUnboundedScrollX() != mScroller.getCurrPos()
          || mOverScrollX != mScroller.getCurrPos()) {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    int myWidthSpec =
        MeasureSpec.makeMeasureSpec(widthSize - mInsets.left - mInsets.right, MeasureSpec.EXACTLY);
    int myHeightSpec =
        MeasureSpec.makeMeasureSpec(heightSize - mInsets.top - mInsets.bottom, MeasureSpec.EXACTLY);
  ...
  /**
   * Initializes {@code outPageScrolls} with scroll positions for view at that index. The length of
   * {@code outPageScrolls} should be same as the the childCount
   */
  protected boolean getPageScrolls(
      int[] outPageScrolls, boolean layoutChildren, ComputePageScrollsLogic scrollLogic) {
    final int childCount = getChildCount();
    ...
    final int verticalCenter =
        (getPaddingTop() + getMeasuredHeight() + mInsets.top - mInsets.bottom - getPaddingBottom())
            / 2;
    ...
    for (int i = startIndex, childLeft = scrollOffsetLeft + offsetForPageScrolls();
      ...
      if (scrollLogic.shouldIncludeView(child)) {
        ...
        final int pageScroll =
            mIsRtl ? (childLeft - scrollOffsetLeft) : Math.max(0, childRight - scrollOffsetRight);
   ...
   *
   * <p>This happens when live folders requery, and if they're off page, they end up calling
      ...
      case MotionEvent.ACTION_UP:
        if (mTouchState == TOUCH_STATE_SCROLLING) {
          ...
          if (!mFreeScroll) {
            ...
            if (((initialScrollX >= mMaxScroll) && (isVelocityXLeft || !isFling))
                || ((initialScrollX <= mMinScroll) && (!isVelocityXLeft || !isFling))) {
              mScroller.springBack(getScrollX(), mMinScroll, mMaxScroll);
              ...
              if (finalX > 0 && finalX < mMaxScroll) {
                ...
                int pageSnappedX =
                    finalX < (firstPageScroll + mMinScroll) / 2
                        ? mMinScroll
                        : finalX > (lastPageScroll + mMaxScroll) / 2
                            ? mMaxScroll
                            : getScrollForPage(mNextPage);

                mScroller.setFinalPos(pageSnappedX);
                // Ensure the scroll/snap doesn't happen too fast;
                int extraScrollDuration =
        ...
        case MotionEvent.ACTION_SCROLL:
          {
            ...
            if (hScroll != 0 || vScroll != 0) {
              boolean isForwardScroll =
                  mIsRtl ? (hScroll < 0 || vScroll < 0) : (hScroll > 0 || vScroll > 0);
              if (isForwardScroll) {
                scrollRight();
              } else {
                scrollLeft();
  ...
  protected boolean snapToPageWithVelocity(int whichPage, int velocity) {
    ...
    float distance =
        halfScreenSize + halfScreenSize * distanceInfluenceForSnapDuration(distanceRatio);
  ...
  public String getCurrentPageDescription() {
    return getContext()
        .getString(R.string.default_scroll_format, getNextPage() + 1, getChildCount());
  }
