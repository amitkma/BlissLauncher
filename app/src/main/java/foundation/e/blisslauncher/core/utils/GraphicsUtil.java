...
public class GraphicsUtil {
  ...
  /**
   * Takes 1 or more drawables and merges them to form a single Drawable. However, if more than 4
   * drawables are provided, only the first 4 are used.
   */
  ...
  /**
   * A utility method that simplifies calls to the generateFolderIcon() method that expects an array
   * of Drawables.
   */
  ...
  public Bitmap addBackground(Bitmap bitmap, boolean isFolder) {
    if (!hasTransparency(bitmap)) {
      bitmap =
          Bitmap.createScaledBitmap(
              bitmap, appIconWidth, (appIconWidth * bitmap.getHeight() / bitmap.getWidth()), true);
      return bitmap;
    }
    ...
    canvas.drawBitmap(
        bitmap, (width - bitmap.getWidth()) / 2, (height - bitmap.getHeight()) / 2, paint);
    return mergedBitmap;
  }
  ...
  public Bitmap addBackground(Drawable appIcon, boolean isFolder) {
    ...
    if (appIcon instanceof BitmapDrawable) {
      ...
      bitmap =
          Bitmap.createBitmap(
              appIcon.getIntrinsicWidth(), appIcon.getIntrinsicHeight(), Bitmap.Config.ARGB_8888);
      Canvas canvas = new Canvas(bitmap);
  ...
  public BitmapDrawable convertToRoundedCorner(Context context, Bitmap src) {
    return new BitmapDrawable(
        context.getResources(), BitmapUtils.getCroppedBitmap(src, DeviceProfile.path));
  }
