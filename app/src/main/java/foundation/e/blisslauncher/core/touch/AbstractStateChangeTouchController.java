...
public abstract class AbstractStateChangeTouchController
   ...
   */
  protected abstract LauncherState getTargetState(
      LauncherState fromState, boolean isDragTowardPositive);
  ...
  private boolean reinitCurrentAnimation(boolean reachedToState, boolean isDragTowardPositive) {
    ...
    int animComponents =
        goingBetweenNormalAndOverview(mFromState, mToState) ? NON_ATOMIC_COMPONENT : ANIM_ALL;
    if (mAtomicAnim != null) {
  ...
  /**
   * When going between normal and overview states, see if we passed the overview threshold and play
   * the appropriate atomic animation if so.
   */
  private void maybeUpdateAtomicAnim(
      LauncherState fromState, LauncherState toState, float progress) {
    if (!goingBetweenNormalAndOverview(fromState, toState)) {
      return;
    }
    float threshold =
        toState == OVERVIEW ? ATOMIC_OVERVIEW_ANIM_THRESHOLD : 1f - ATOMIC_OVERVIEW_ANIM_THRESHOLD;
  ...
  private AnimatorSet createAtomicAnimForState(
      LauncherState fromState, LauncherState targetState, long duration) {
    AnimatorSetBuilder builder = getAnimatorSetBuilderForStates(fromState, targetState);
    return mLauncher
        .getStateManager()
        .createAtomicAnimation(
            fromState, targetState, builder, ATOMIC_OVERVIEW_SCALE_COMPONENT, duration);
  }

  protected AnimatorSetBuilder getAnimatorSetBuilderForStates(
  ...
  public void onDragEnd(float velocity, boolean fling) {
    ...
    final int durationMultiplier =
        blockedFling && targetState == mFromState
            ? LauncherAnimUtils.blockedFlingDurationFactor(velocity)
            : 1;
    ...
    if (targetState == mToState) {
      ...
      if (progress >= 1) {
        ...
        duration =
            SwipeDetector.calculateDuration(velocity, endProgress - Math.max(progress, 0))
                * durationMultiplier;
      }
    } else {
      ...
      if (progress <= 0) {
        ...
        startProgress =
            Utilities.boundToRange(
                progress + velocity * SINGLE_FRAME_MS * mProgressMultiplier, 0f, 1f);
        duration =
            SwipeDetector.calculateDuration(velocity, Math.min(progress, 1) - endProgress)
                * durationMultiplier;
      }
    }
   ...
   *
   * <p>Note that this only applies when we are controlling the atomic components separately from
  ...
  protected void updateSwipeCompleteAnimation(
      ...
      float velocity,
      boolean isFling) {
    animator.setDuration(expectedDuration).setInterpolator(scrollInterpolatorForVelocity(velocity));
  }
