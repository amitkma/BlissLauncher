 ...
 *
 * <p>Definition of swipe is different from android system in that this detector handles 'swipe to
...
public class SwipeDetector {
  ...
  public abstract static class Direction {
    ...
    abstract float getDisplacement(
        MotionEvent ev, int pointerIndex, PointF refPoint, boolean isRtl);
  ...
  private void reportDragEnd() {
    ...
    if (DBG) {
      Log.d(TAG, String.format("onScrollEnd disp=%.1f, velocity=%.1f", mDisplacement, velocity));
    }
  ...
  public static long calculateDuration(float velocity, float progressNeeded) {
    ...
    if (DBG) {
      Log.d(
          TAG,
          String.format("calculateDuration=%d, v=%f, d=%f", duration, velocity, progressNeeded));
    }
    return duration;
  }
}
