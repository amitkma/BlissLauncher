...
public class HorizontalPager extends ViewGroup implements Insettable {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    for (int i = 0; i < count; i++) {
      getChildAt(i)
          .measure(MeasureSpec.makeMeasureSpec(pageWidth, MeasureSpec.EXACTLY), heightMeasureSpec);
    }
  ...
  private boolean inThresholdRegion() {
    return (mLastMotionRawY
            / BlissLauncher.getApplication(getContext()).getDeviceProfile().availableHeightPx)
        > (float) 1 / 5;
  }
  ...
  public void setInsets(Rect insets) {
    ...
    InsettableRelativeLayout.LayoutParams lp =
        (InsettableRelativeLayout.LayoutParams) getLayoutParams();
  ...
  public interface OnScrollListener {
    /**
     * Receives the current scroll X value. This value will be adjusted to assume the left edge of
     * the first page has a scroll position of 0. Note that values less than 0 and greater than the
     * right edge of the last page are possible due to touch events scrolling beyond the edges.
     *
