...
import static foundation.e.blisslauncher.features.test.anim.Interpolators.SCROLL;
...
public class OverScroller {
   ...
   * @param interpolator The scroll interpolator. If null, a default (viscous) interpolator will be
   *     used.
   */
   ...
   * used with {@link #setFinalPos(int)}.
   *
   ...
   *
   * @param start Starting horizontal scroll offset in pixels. Positive numbers will scroll the
   *     content to the left.
   * @param delta Distance to travel. Positive numbers will scroll the content to the left.
   */
   ...
   *
   * @param start Starting scroll offset in pixels. Positive numbers will scroll the content to the
   *     left.
   * @param delta Distance to travel. Positive numbers will scroll the content to the left.
   ...
   * @param velocity Initial velocity of the fling (X) measured in pixels per second.
   * @param min Minimum X value. The scroller will not scroll past this point unless overX > 0. If
   *     overfling is allowed, it will use minX as a springback boundary.
   * @param max Maximum X value. The scroller will not scroll past this point unless overX > 0. If
   *     overfling is allowed, it will use maxX as a springback boundary.
   * @param over Overfling range. If > 0, horizontal overfling in either direction will be possible.
   */
  ...
  /**
   * Returns whether the current Scroller is currently returning to a valid position. Valid bounds
   * were provided by the {@link #fling(int, int, int, int, int)} method.
   *
   * <p>One should check this value before calling {@link #startScroll(int, int)} as the
   * interpolation currently in progress to restore a valid position will then be stopped. The
   * caller has to take into account the fact that the started scroll will start from an
   * overscrolled position.
   *
   * @return true when the current position is overscrolled and in the process of interpolating back
   *     to a valid value.
   */
  ...
  static class SplineOverScroller {
    ...
    void startScroll(int start, int distance, int duration, float velocity) {
      ...
      if (mState == SPRING) {
        ...
        mSpring.setSpring(
            new SpringForce(mFinal)
                .setStiffness(SpringForce.STIFFNESS_LOW)
                .setDampingRatio(SpringForce.DAMPING_RATIO_LOW_BOUNCY));
        mSpring.setStartVelocity(velocity);
        mSpring.animateToFinalPosition(mFinal);
        mSpring.addEndListener(
            (animation, canceled, value, velocity1) -> {
              finish();
              mState = SPLINE;
              mSpring = null;
            });
