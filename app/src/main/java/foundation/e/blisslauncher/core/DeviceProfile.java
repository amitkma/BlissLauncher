...
public class DeviceProfile {
  ...
  private void updateIconSize(float scale, Resources res, DisplayMetrics dm) {
    ...
    uninstallIconSizePx =
        (tempUninstallIconSize > iconDrawablePaddingPx)
    ...
    dateTextTopPadding =
        (dateTextviewHeight
                - (int) (1.14 * Utilities.calculateTextHeight((float) dateTextSize / 2)))
            / 2;
    dateTextBottomPadding =
        (dateTextviewHeight
                - (int) (0.86 * Utilities.calculateTextHeight((float) dateTextSize / 2)))
            / 2;

    cellHeightWithoutPaddingPx =
        iconSizePx + Utilities.pxFromDp(4, dm) + Utilities.calculateTextHeight(iconTextSizePx);
  ...
  public Path getRoundedCornerPath(int iconSize) {
    return resizePath(
        PathParser.createPathFromPathData(AdaptiveIconUtils.getMaskPath()), iconSize, iconSize);
  }
  ...
  public interface OnDeviceProfileChangeListener {
    ...
    /**
     * Called when the device profile is reassigned. Note that for layout and measurements, it is
     * sufficient to listen for inset changes. Use this callback when you need to perform a one time
     * operation.
     */
