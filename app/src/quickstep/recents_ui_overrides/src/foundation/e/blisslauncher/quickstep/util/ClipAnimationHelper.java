...
public class ClipAnimationHelper {
  ...
  private BiFunction<RemoteAnimationTargetCompat, Float, Float> mTaskAlphaCallback = (t, a1) -> a1;
  ...
  public void updateTargetRect(Rect targetRect) {
    ...
    mTargetRect.offset(
        mHomeStackBounds.left - mSourceStackBounds.left,
    ...
    Utilities.scaleRectFAboutCenter(scaledTargetRect, mSourceRect.width() / mTargetRect.width());
  ...
  public void prepareAnimation(VariantDeviceProfile dp, boolean isOpening) {
  ...
  public RectF applyTransform(
      RemoteAnimationTargetSet targetSet, TransformParams params, boolean launcherOnTop) {
    ...
    if (params.currentRect == null) {
      ...
      mClipRectF.right = mSourceStackBounds.width() - (sourceWindowClipInsets.right * progress);
      mClipRectF.bottom = mSourceStackBounds.height() - (sourceWindowClipInsets.bottom * progress);
    ...
    for (int i = 0; i < targetSet.unfilteredApps.length; i++) {
        ...
        if (app.activityType != RemoteAnimationTargetCompat.ACTIVITY_TYPE_HOME) {
          ...
          if (mSupportsRoundedCornersOnWindows) {
            if (params.cornerRadius > -1) {
              ...
              float windowCornerRadius = mUseRoundedCornersOnWindows ? mWindowCornerRadius : 0;
              cornerRadius = Utilities.mapRange(progress, windowCornerRadius, mTaskCornerRadius);
            }
      ...
      } else if (ENABLE_QUICKSTEP_LIVE_TILE && launcherOnTop) {
        crop = null;
      ...
      surfaceParams[i] =
          new SurfaceParams(app.leash, alpha, mTmpMatrix, crop, layer, cornerRadius / scale);
    }
  ...
  public void setTaskAlphaCallback(BiFunction<RemoteAnimationTargetCompat, Float, Float> callback) {
  ...
  public void fromTaskThumbnailView(
      TaskThumbnailView ttv, RecentsView rv, @Nullable RemoteAnimationTargetCompat target) {
  ...
  private void updateStackBoundsToMultiWindowTaskSize(BaseDraggingActivity activity) {
    ...
    int halfDividerSize =
        activity.getResources().getDimensionPixelSize(R.dimen.multi_window_task_divider_size) / 2;
