...
import static foundation.e.blisslauncher.features.test.anim.LauncherAnimUtils.VIEW_TRANSLATE_Y;
import static foundation.e.blisslauncher.quickstep.TaskUtils.checkCurrentOrManagedUserId;
import static foundation.e.blisslauncher.uioverrides.touchcontrollers.TaskViewTouchController.SUCCESS_TRANSITION_PROGRESS;
...
import foundation.e.blisslauncher.core.utils.OverScroller;
...
import foundation.e.blisslauncher.quickstep.RecentsAnimationWrapper;
import foundation.e.blisslauncher.quickstep.RecentsModel;
import foundation.e.blisslauncher.quickstep.RecentsModel.TaskThumbnailChangeListener;
import foundation.e.blisslauncher.quickstep.TaskThumbnailCache;
...
import foundation.e.blisslauncher.quickstep.util.ViewPool;
import java.util.ArrayList;
import java.util.function.Consumer;

/** A list of recent tasks. */
@TargetApi(Build.VERSION_CODES.P)
public abstract class RecentsView<T extends BaseActivity> extends PagedView
    implements Insettable,
        TaskThumbnailCache.HighResLoadingState.HighResLoadingStateChangedCallback,
        TaskThumbnailChangeListener {
          ...
          BackgroundExecutor.get()
              .submit(
                  () -> {
                    ...
                    // TODO: Add callbacks from AM reflecting adding/removing from the recents list,
                    // and
                    //       remove all these checks
                    Task.TaskKey taskKey = taskView.getTask().key;
                    if (PackageManagerWrapper.getInstance()
                            .getActivityInfo(taskKey.getComponent(), taskKey.userId)
                        == null) {
                      // The package was uninstalled
                      handler.post(
                          () -> dismissTask(taskView, true /* animate */, false /* removeTask */));
                    } else {
                      mModel.findTaskWithId(
                          taskKey.id,
                          (key) -> {
                            if (key == null) {
                              ...
                              handler.post(
                                  () ->
                                      dismissTask(
                                          taskView, true /* animate */, false /* removeTask */));
                            }
                          });
                    }
                  });
  ...
  public RecentsView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    mClearAllButton =
        (ClearAllButton)
            LayoutInflater.from(context).inflate(R.layout.overview_clear_all_button, this, false);
    mClearAllButton.setOnClickListener(this::dismissAllTasks);

    mTaskViewPool =
        new ViewPool<>(context, this, R.layout.task, 20 /* max size */, 10 /* initial size */);
  ...
  public boolean onTouchEvent(MotionEvent ev) {
    ...
    switch (ev.getAction()) {
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        if (mTouchDownToStartHome
            && (isHandlingTouch() || squaredHypot(mDownX - x, mDownY - y) > mSquaredTouchSlop)) {
          mTouchDownToStartHome = false;
        }
        break;
        ...
        if (!isHandlingTouch()) {
          if (mShowEmptyMessage) {
            ...
            final boolean clearAllButtonDeadZoneConsumed =
                mClearAllButton.getAlpha() == 1 && mClearAllButtonDeadZoneRect.contains(x, y);
  ...
  private void applyLoadPlan(ArrayList<Task> tasks) {
    ...
    TaskView ignoreRestTaskView = mIgnoreResetTaskId == -1 ? null : getTaskView(mIgnoreResetTaskId);
  ...
  private void updateTaskStackListenerState() {
    boolean handleTaskStackChanges =
        mOverviewStateEnabled && isAttachedToWindow() && getWindowVisibility() == VISIBLE;
  ...
  public void setInsets(Rect insets) {
    ...
    setPadding(
        mTempRect.left - mInsets.left,
        mTempRect.top - mInsets.top,
  ...
  public void updateCurveProperties() {
    ...
    mScrollState.scrollFromEdge = mIsRtl ? scrollX : (mMaxScroll - scrollX);
    ...
    for (int i = 0; i < pageCount; i++) {
      ...
      mScrollState.linearInterpolation =
          Math.min(1, Math.abs(distanceFromScreenCenter) / distanceToReachEdge);
      ((PageCallbacks) page).onPageScroll(mScrollState);
    }
  }

  /**
   * Iterates through all thet asks, and loads the associated task data for newly visible tasks, and
   * unloads the associated task data for tasks that are no longer visible.
   */
  ...
  /**
   * Called only when a swipe-up gesture from an app has completed. Only called after {@link
   * #onGestureAnimationStart} and {@link #onGestureAnimationEnd()}.
   */
  public void onSwipeUpAnimationSuccess() {
    if (getRunningTaskView() != null) {
      float startProgress =
          ENABLE_QUICKSTEP_LIVE_TILE ? mLiveTileOverlay.cancelIconAnimation() : 0f;
   ...
   *
   * <p>All subsequent calls to reload will keep the task as the first item until {@link #reset()}
      ...
      mLayoutTransition.addTransitionListener(
          new TransitionListener() {
            ...
            public void startTransition(
                LayoutTransition transition, ViewGroup viewGroup, View view, int i) {}
  ...
  public static class ScrollState {
    ...
    /**
     * The progress from 0 to 1, where 0 is the center of the screen and 1 is the edge of the
     * screen.
     */
  ...
  private void addDismissedTaskAnimations(View taskView, AnimatorSet anim, long duration) {
    ...
    if (QUICKSTEP_SPRINGS && taskView instanceof TaskView)
      addAnim(
          new SpringObjectAnimator<>(
              taskView,
              VIEW_TRANSLATE_Y,
      ...
      addAnim(
          ObjectAnimator.ofFloat(taskView, TRANSLATION_Y, -taskView.getHeight()),
          duration,
          LINEAR,
          anim);
    }
  ...
  private void removeTask(
      Task task, int index, PendingAnimation.OnEndListener onEndListener, boolean shouldLog) {
    if (task != null) {
      ActivityManagerWrapper.getInstance().removeTask(task.key.id);
    }
  }

  public PendingAnimation createTaskDismissAnimation(
      TaskView taskView, boolean animateTaskView, boolean shouldRemoveTask, long duration) {
    if (mPendingAnimation != null) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (child == taskView) {
        ...
        if (scrollDiff != 0) {
          if (QUICKSTEP_SPRINGS && child instanceof TaskView) {
            addAnim(
                new SpringObjectAnimator<>(
                    child,
                    VIEW_TRANSLATE_X,
            ...
            addAnim(
                ObjectAnimator.ofFloat(child, TRANSLATION_X, scrollDiff), duration, ACCEL, anim);
          }
    ...
    mPendingAnimation.addEndListener(
        new Consumer<PendingAnimation.OnEndListener>() {
          @Override
          public void accept(PendingAnimation.OnEndListener onEndListener) {
            if (ENABLE_QUICKSTEP_LIVE_TILE && taskView.isRunningTask() && onEndListener.isSuccess) {
  ...
  private static void addAnim(
      Animator anim, long duration, TimeInterpolator interpolator, AnimatorSet set) {
  ...
  private void runDismissAnimation(PendingAnimation pendingAnim) {
    AnimatorPlaybackController controller =
        AnimatorPlaybackController.wrap(pendingAnim.anim, DISMISS_TASK_DURATION);
  ...
  public void dismissTask(TaskView taskView, boolean animateTaskView, boolean removeTask) {
    runDismissAnimation(
        createTaskDismissAnimation(taskView, animateTaskView, removeTask, DISMISS_TASK_DURATION));
  }
  ...
  /**
   * Freezes the view visibility change. When frozen, the view will not change its visibility to
   * gone due to alpha changes.
   */
  ...
  public void updateEmptyMessage() {
    ...
    boolean hasSizeChanged = mLastMeasureSize.x != getWidth() || mLastMeasureSize.y != getHeight();
  ...
  protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
    ...
    setPivotY(
        ((mInsets.top + getPaddingTop() + mTaskTopMargin)
                + (getHeight() - mInsets.bottom - getPaddingBottom()))
            / 2);
    setPivotX(
        ((mInsets.left + getPaddingLeft()) + (getWidth() - mInsets.right - getPaddingRight())) / 2);
  }
  ...
  private void updateDeadZoneRects() {
    ...
    if (count > 0) {
      ...
      mTaskViewDeadZoneRect.union(
          taskView.getLeft(), taskView.getTop(), taskView.getRight(), taskView.getBottom());
    }
  }
    ...
    if (mShowEmptyMessage && hasValidSize && mEmptyTextLayout == null) {
      ...
      mEmptyTextLayout =
          StaticLayout.Builder.obtain(
                  mEmptyMessage, 0, mEmptyMessage.length(), mEmptyMessagePaint, availableWidth)
              .setAlignment(Layout.Alignment.ALIGN_CENTER)
              .build();
      int totalHeight =
          mEmptyTextLayout.getHeight() + mEmptyMessagePadding + mEmptyIcon.getIntrinsicHeight();

      int top = (mLastMeasureSize.y - totalHeight) / 2;
      int left = (mLastMeasureSize.x - mEmptyIcon.getIntrinsicWidth()) / 2;
      mEmptyIcon.setBounds(
          left, top, left + mEmptyIcon.getIntrinsicWidth(), top + mEmptyIcon.getIntrinsicHeight());
    }
  }
  ...
  protected void maybeDrawEmptyMessage(Canvas canvas) {
    if (mShowEmptyMessage && mEmptyTextLayout != null) {
      ...
      mTempRect.set(
          mInsets.left + getPaddingLeft(),
          mInsets.top + getPaddingTop(),
          mInsets.right + getPaddingRight(),
          mInsets.bottom + getPaddingBottom());
      canvas.save();
      canvas.translate(
          getScrollX() + (mTempRect.left - mTempRect.right) / 2,
   ...
   *
   * <p>If launching one of the adjacent tasks, parallax the center task and other adjacent task to
   * the right.
   */
  public AnimatorSet createAdjacentPageAnimForTaskLaunch(
    ...
    LauncherState.ScaleAndTranslation toScaleAndTranslation =
        clipAnimationHelper.getScaleAndTranslation();
    ...
    if (launchingCenterTask) {
      ...
      anim.play(
          ObjectAnimator.ofFloat(
              getPageAt(centerTaskIndex), TRANSLATION_X, mIsRtl ? -displacementX : displacementX));
  ...
  public PendingAnimation createTaskLauncherAnimation(TaskView tv, long duration) {
    ...
    mPendingAnimation.addEndListener(
        (onEndListener) -> {
          if (onEndListener.isSuccess) {
            Consumer<Boolean> onLaunchResult =
                (result) -> {
  ...
  public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
    ...
    final AccessibilityNodeInfo.CollectionInfo collectionInfo =
        AccessibilityNodeInfo.CollectionInfo.obtain(
            1, getTaskViewCount(), false, AccessibilityNodeInfo.CollectionInfo.SELECTION_MODE_NONE);
