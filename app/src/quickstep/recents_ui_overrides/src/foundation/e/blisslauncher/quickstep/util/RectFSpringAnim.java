...
import foundation.e.blisslauncher.features.test.anim.FlingSpringAnim;
import java.util.ArrayList;
import java.util.List;

/**
 * Applies spring forces to animate from a starting rect to a target rect, while providing update
...
public class RectFSpringAnim {
  ...
  public void start(PointF velocityPxPerMs) {
    ...
    OnAnimationEndListener onXEndListener =
        ((animation, canceled, centerX, velocityX) -> {
          mRectXAnimEnded = true;
          maybeOnEnd();
        });
    OnAnimationEndListener onYEndListener =
        ((animation, canceled, centerY, velocityY) -> {
  ...
  private void onUpdate() {
    if (!mOnUpdateListeners.isEmpty()) {
      ...
      float currentHeight =
          Utilities.mapRange(mCurrentScaleProgress, mStartRect.height(), mTargetRect.height());
      if (mTrackingBottomY) {
        mCurrentRect.set(
