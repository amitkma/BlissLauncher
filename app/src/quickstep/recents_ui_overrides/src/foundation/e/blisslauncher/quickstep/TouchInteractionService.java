...
import foundation.e.blisslauncher.quickstep.SysUINavigationMode.NavigationModeChangeListener;
import foundation.e.blisslauncher.quickstep.inputconsumers.AccessibilityInputConsumer;
import foundation.e.blisslauncher.quickstep.inputconsumers.AssistantTouchConsumer;
import foundation.e.blisslauncher.quickstep.inputconsumers.DeviceLockedInputConsumer;
import foundation.e.blisslauncher.quickstep.inputconsumers.FallbackNoButtonInputConsumer;
import foundation.e.blisslauncher.quickstep.inputconsumers.InputConsumer;
import foundation.e.blisslauncher.quickstep.inputconsumers.OtherActivityInputConsumer;
import foundation.e.blisslauncher.quickstep.inputconsumers.OverviewInputConsumer;
import foundation.e.blisslauncher.quickstep.inputconsumers.OverviewWithoutFocusInputConsumer;
import foundation.e.blisslauncher.quickstep.inputconsumers.ResetGestureInputConsumer;
import foundation.e.blisslauncher.quickstep.inputconsumers.ScreenPinnedInputConsumer;
...
public class TouchInteractionService extends Service
    implements NavigationModeChangeListener, DisplayListener {
  ...
  private final IBinder mMyBinder =
      new IOverviewProxy.Stub() {
        ...
        public void onAssistantVisibilityChanged(float visibility) {
          ...
          MAIN_THREAD_EXECUTOR.execute(TouchInteractionService.this::onAssistantVisibilityChanged);
        }

        public void onBackAction(
            boolean completed, int downX, int downY, boolean isButton, boolean gestureSwipeLeft) {
  ...
  public void onCreate() {
    ...
    mDefaultDisplayId = getSystemService(WindowManager.class).getDefaultDisplay().getDisplayId();
    String blockingActivity = getString(R.string.gesture_blocking_activity);
    mGestureBlockingActivity =
        TextUtils.isEmpty(blockingActivity)
            ? null
            : ComponentName.unflattenFromString(blockingActivity);

    mExclusionListener =
        new SystemGestureExclusionListenerCompat(mDefaultDisplayId) {
          @Override
          @BinderThread
  ...
  private void initInputMonitor() {
    ...
    try {
      mInputMonitorCompat =
          InputMonitorCompat.fromBundle(
              mISystemUiProxy.monitorGestureInput("swipe-up", mDefaultDisplayId),
              KEY_EXTRA_INPUT_MONITOR);
      mInputEventReceiver =
          mInputMonitorCompat.getInputReceiver(
              Looper.getMainLooper(), mMainChoreographer, this::onInputEvent);
  ...
  private void initTouchBounds() {
    ...
    if (mMode == Mode.NO_BUTTON) {
      ...
      final int assistantWidth =
          getResources().getDimensionPixelSize(R.dimen.gestures_assistant_width);
      final float assistantHeight =
          Math.max(touchHeight, QuickStepContract.getWindowCornerRadius(getResources()));
      mAssistantLeftRegion.bottom = mAssistantRightRegion.bottom = mSwipeTouchRegion.bottom;
      mAssistantLeftRegion.top =
          mAssistantRightRegion.top = mSwipeTouchRegion.bottom - assistantHeight;
      ...
      switch (defaultDisplay.getRotation()) {
        case Surface.ROTATION_90:
          mSwipeTouchRegion.left =
              mSwipeTouchRegion.right
        ...
        case Surface.ROTATION_270:
          ...
          break;
        default:
          mSwipeTouchRegion.top =
              mSwipeTouchRegion.bottom - getNavbarSize(ResourceUtils.NAVBAR_BOTTOM_GESTURE_SIZE);
      }
    }
  }
    ...
    if (mMode.hasGestures != newMode.hasGestures) {
      if (newMode.hasGestures) {
        getSystemService(DisplayManager.class)
            .registerDisplayListener(this, MAIN_THREAD_EXECUTOR.getHandler());
      } else {
  ...
  private void onAssistantVisibilityChanged() {
    if (mIsUserUnlocked) {
      mOverviewComponentObserver
          ...
          .onAssistantVisibilityChanged(mLastAssistantVisibility);
    }
  }
  ...
  private InputConsumer newConsumer(boolean useSharedState, MotionEvent event) {
    ...
    InputConsumer base =
        isInValidSystemUiState || useSharedState
            ? newBaseConsumer(useSharedState, event)
    ...
    if (mMode == Mode.NO_BUTTON) {
      ...
      if (canTriggerAssistantAction(event)) {
        base =
            new AssistantTouchConsumer(
                this, mISystemUiProxy, activityControl, base, mInputMonitorCompat);
      }
      ...
      if ((mSystemUiStateFlags & SYSUI_STATE_A11Y_BUTTON_CLICKABLE) != 0) {
        base =
            new AccessibilityInputConsumer(
                this,
                mISystemUiProxy,
  ...
  private InputConsumer newBaseConsumer(boolean useSharedState, MotionEvent event) {
    ...
    if (runningTaskInfo == null
    ...
    } else if (ENABLE_QUICKSTEP_LIVE_TILE && activityControl.isInLiveTileMode()) {
      return createOverviewInputConsumer(event);
      ...
      return new FallbackNoButtonInputConsumer(
          this,
          activityControl,
          mInputMonitorCompat,
          sSwipeSharedState,
  ...
  private boolean disableHorizontalSwipe(MotionEvent event) {
    ...
    return mMode == Mode.NO_BUTTON
        && exclusionRegion != null
        && exclusionRegion.contains((int) event.getX(), (int) event.getY());
  }

  private OtherActivityInputConsumer createOtherActivityInputConsumer(
  ...
  private InputConsumer createDeviceLockedInputConsumer(RunningTaskInfo taskInfo) {
    if (mMode == Mode.NO_BUTTON && taskInfo != null) {
      return new DeviceLockedInputConsumer(
          this, sSwipeSharedState, mInputMonitorCompat, mSwipeTouchRegion, taskInfo.taskId);
    } else {
      return mResetGestureInputConsumer;
    }
  }
  ...
  public InputConsumer createOverviewInputConsumer(MotionEvent event) {
    ...
    if (activity.getRootView().hasWindowFocus() || sSwipeSharedState.goingToLauncher) {
      ...
      return new OverviewWithoutFocusInputConsumer(
          this, mInputMonitorCompat, disableHorizontalSwipe(event));
    }
  }
