...
import static foundation.e.blisslauncher.features.test.BaseActivity.STATE_HANDLER_INVISIBILITY_FLAGS;
import static foundation.e.blisslauncher.features.test.SystemUiController.UI_STATE_OVERVIEW;
import static foundation.e.blisslauncher.features.test.anim.Interpolators.ACCEL_1_5;
...
import static foundation.e.blisslauncher.quickstep.ActivityControlHelper.AnimationFactory.ShelfAnimState.HIDE;
import static foundation.e.blisslauncher.quickstep.ActivityControlHelper.AnimationFactory.ShelfAnimState.PEEK;
...
import static foundation.e.blisslauncher.quickstep.WindowTransformSwipeHandler.GestureEndTarget.HOME;
import static foundation.e.blisslauncher.quickstep.WindowTransformSwipeHandler.GestureEndTarget.LAST_TASK;
import static foundation.e.blisslauncher.quickstep.WindowTransformSwipeHandler.GestureEndTarget.NEW_TASK;
import static foundation.e.blisslauncher.quickstep.WindowTransformSwipeHandler.GestureEndTarget.RECENTS;
...
import foundation.e.blisslauncher.quickstep.ActivityControlHelper.AnimationFactory;
import foundation.e.blisslauncher.quickstep.ActivityControlHelper.AnimationFactory.ShelfAnimState;
import foundation.e.blisslauncher.quickstep.ActivityControlHelper.HomeAnimationFactory;
import foundation.e.blisslauncher.quickstep.SysUINavigationMode.Mode;
import foundation.e.blisslauncher.quickstep.inputconsumers.InputConsumer;
import foundation.e.blisslauncher.quickstep.inputconsumers.OverviewInputConsumer;
import foundation.e.blisslauncher.quickstep.util.ClipAnimationHelper;
import foundation.e.blisslauncher.quickstep.util.RectFSpringAnim;
...
import foundation.e.blisslauncher.quickstep.views.LiveTileOverlay;
...
public class WindowTransformSwipeHandler<T extends BaseDraggingActivity>
    implements SwipeAnimationTargetSet.SwipeAnimationListener, OnApplyWindowInsetsListener {
  ...
  public enum GestureEndTarget {
    ...
    RECENTS(
        1,
        STATE_SCALED_CONTROLLER_RECENTS | STATE_CAPTURE_SCREENSHOT | STATE_SCREENSHOT_VIEW_SHOWN,
        true,
        false,
        true),
  ...
  private VariantDeviceProfile mDp;
  ...
  public WindowTransformSwipeHandler(
    ...
    mActivityInitListener = mActivityControlHelper.createActivityInitListener(this::onActivityInit);
  ...
  private void initStateCallbacks() {
    ...
    mStateCallback.addCallback(
        STATE_LAUNCHER_PRESENT | STATE_GESTURE_STARTED, this::onLauncherPresentAndGestureStarted);

    mStateCallback.addCallback(
        STATE_LAUNCHER_DRAWN | STATE_GESTURE_STARTED, this::initializeLauncherAnimationController);

    mStateCallback.addCallback(
        STATE_LAUNCHER_PRESENT | STATE_LAUNCHER_DRAWN, this::launcherFrameDrawn);
    ...
    mStateCallback.addCallback(
        STATE_LAUNCHER_STARTED | STATE_APP_CONTROLLER_RECEIVED,
        this::sendRemoteAnimationsToAnimationFactory);

    mStateCallback.addCallback(
        STATE_RESUME_LAST_TASK | STATE_APP_CONTROLLER_RECEIVED, this::resumeLastTask);
    mStateCallback.addCallback(
        STATE_START_NEW_TASK | STATE_SCREENSHOT_CAPTURED, this::startNewTask);
    ...
    mStateCallback.addCallback(
        STATE_SCREENSHOT_CAPTURED | STATE_GESTURE_COMPLETED | STATE_SCALED_CONTROLLER_HOME,
        this::finishCurrentTransitionToHome);
    mStateCallback.addCallback(
        STATE_SCALED_CONTROLLER_HOME | STATE_CURRENT_TASK_FINISHED, this::reset);
    ...
    mStateCallback.addCallback(
        STATE_LAUNCHER_PRESENT | STATE_HANDLER_INVALIDATED, this::invalidateHandlerWithLauncher);
    mStateCallback.addCallback(
        STATE_HANDLER_INVALIDATED | STATE_RESUME_LAST_TASK, this::notifyTransitionCancelled);

    mStateCallback.addCallback(
        STATE_APP_CONTROLLER_RECEIVED | STATE_GESTURE_STARTED,
        mRecentsAnimationWrapper::enableInputConsumer);

    if (!ENABLE_QUICKSTEP_LIVE_TILE) {
      mStateCallback.addChangeHandler(
          STATE_APP_CONTROLLER_RECEIVED
              | STATE_LAUNCHER_PRESENT
  ...
  private void initTransitionEndpoints(VariantDeviceProfile dp) {
    mDp = dp;

    Rect tempRect = new Rect();
    mTransitionDragLength =
        mActivityControlHelper.getSwipeUpDestinationAndLength(dp, mContext, tempRect);
    mClipAnimationHelper.updateTargetRect(tempRect);
    if (mMode == Mode.NO_BUTTON) {
      // We can drag all the way to the top of the screen.
      mDragLengthFactor = (float) dp.getHeightPx() / mTransitionDragLength;
    }
  }
  ...
  private boolean onActivityInit(final T activity, Boolean alreadyOnHome) {
    ...
    SyncRtSurfaceTransactionApplierCompat.create(
        mRecentsView,
        applier -> {
    ...
    mRecentsView.setOnScrollChangeListener(
  ...
  private void onLauncherStart(final T activity) {
    ...
    if (mGestureEndTarget != HOME) {
      Runnable initAnimFactory =
          () -> {
            mAnimationFactory =
                mActivityControlHelper.prepareRecentsUI(
                    mActivity,
                    mWasLauncherAlreadyVisible,
                    true,
  ...
  private void initializeLauncherAnimationController() {
    ...
    RecentsModel.INSTANCE
        .get(mContext)
        .getThumbnailCache()
        .getHighResLoadingState()
        .setVisible(true);
  }

  private float getTaskCurveScaleForOffsetX(float offsetX, float taskWidth) {
    float distanceToReachEdge =
        mDp.getWidthPx() / 2
            + taskWidth / 2
  ...
  public void updateDisplacement(float displacement) {
    ...
    if (displacement > mTransitionDragLength * mDragLengthFactor && mTransitionDragLength > 0) {
      ...
      if (shift > DRAG_LENGTH_FACTOR_START_PULLBACK) {
        float pullbackProgress =
            Utilities.getProgress(shift, DRAG_LENGTH_FACTOR_START_PULLBACK, mDragLengthFactor);
        pullbackProgress = PULLBACK_INTERPOLATOR.getInterpolation(pullbackProgress);
        shift =
            DRAG_LENGTH_FACTOR_START_PULLBACK
                + pullbackProgress
  ...
  /**
   * Determines whether to show or hide RecentsView. The window is always synchronized with its
   * corresponding TaskView in RecentsView, so if RecentsView is shown, it will appear to be
   * attached to the window.
   *
   * <p>Note this method has no effect unless the navigation mode is NO_BUTTON.
   */
  private void maybeUpdateRecentsAttachedState(boolean animate) {
    ...
    RemoteAnimationTargetCompat runningTaskTarget =
        mRecentsAnimationWrapper.targetSet == null
            ? null
    ...
    if (mGestureEndTarget != null) {
      ...
      if (animate) {
        ...
        animate =
            (adjacentTask1 != null && adjacentTask1.getGlobalVisibleRect(TEMP_RECT))
  ...
  public void setShelfState(ShelfAnimState shelfState, Interpolator interpolator, long duration) {
    ...
    if (mRecentsView != null && shelfState.shouldPreformHaptic) {
      mRecentsView.performHapticFeedback(
  ...
  private void updateFinalShift() {
    ...
    if (controller != null) {
      ...
      float offsetScale =
          getTaskCurveScaleForOffsetX(offsetX, mClipAnimationHelper.getTargetRect().width());
      mTransformParams.setProgress(shift).setOffsetX(offsetX).setOffsetScale(offsetScale);
      mClipAnimationHelper.applyTransform(mRecentsAnimationWrapper.targetSet, mTransformParams);
      updateSysUiFlags(shift);
    }

    if (ENABLE_QUICKSTEP_LIVE_TILE) {
      if (mRecentsAnimationWrapper.getController() != null) {
        mLiveTileOverlay.update(
  ...
  private void updateLauncherTransitionProgress() {
    ...
    mLauncherTransitionController.setPlayFraction(
        progress <= mShiftAtGestureStart || mShiftAtGestureStart >= 1
            ? 0
            : (progress - mShiftAtGestureStart) / (1 - mShiftAtGestureStart));
  }

  /** @param windowProgress 0 == app, 1 == overview */
  private void updateSysUiFlags(float windowProgress) {
    if (mRecentsView != null) {
      TaskView centermostTask =
          mRecentsView.getTaskViewAt(mRecentsView.getPageNearestToCenterOfScreen());
      int centermostTaskFlags =
          centermostTask == null ? 0 : centermostTask.getThumbnail().getSysUiStatusNavFlags();
      boolean useHomeScreenFlags = windowProgress > 1 - RecentsView.UPDATE_SYSUI_FLAGS_THRESHOLD;
      // We will handle the sysui flags based on the centermost task view.
      mRecentsAnimationWrapper.setWindowThresholdCrossed(
          centermostTaskFlags != 0 || useHomeScreenFlags);
  ...
  public void onRecentsAnimationStart(SwipeAnimationTargetSet targetSet) {
    VariantDeviceProfile dp =
        BlissLauncher.getApplication(mContext)
    ...
    if (targetSet.minimizedHomeBounds != null && runningTaskTarget != null) {
      overviewStackBounds =
          mActivityControlHelper.getOverviewWindowBounds(
              targetSet.minimizedHomeBounds, runningTaskTarget);
      dp =
          dp.getMultiWindowProfile(
              mContext,
              new Point(
      ...
      if (mActivity != null) {
        ...
        overviewStackBounds =
            new Rect(loc[0], loc[1], loc[0] + rootView.getWidth(), loc[1] + rootView.getHeight());
      } else {
        overviewStackBounds = new Rect(0, 0, dp.getWidthPx(), dp.getHeightPx());
      }
  ...
  private InputConsumer createNewInputProxyHandler() {
    ...
    return activity == null ? InputConsumer.NO_OP : new OverviewInputConsumer(activity, null, true);
  }
  ...
  private GestureEndTarget calculateEndTarget(
      PointF velocity, float endVelocity, boolean isFling, boolean isCancel) {
      ...
      if (isCancel) {
        ...
        endTarget =
            reachedOverviewThreshold && mGestureStarted
                ? RECENTS
                : goingToNewTask ? NEW_TASK : LAST_TASK;
      }
    } else {
  ...
  private void handleNormalGestureEnd(
      float endVelocity, boolean isFling, PointF velocity, boolean isCancel) {
    ...
    if (!isFling) {
      long expectedDuration =
          Math.abs(
              Math.round(
                  (endShift - currentShift) * MAX_SWIPE_DURATION * SWIPE_DURATION_MULTIPLIER));
      ...
      startShift =
          Utilities.boundToRange(
              currentShift - velocityPxPerMs.y * SINGLE_FRAME_MS / mTransitionDragLength,
              0,
              mDragLengthFactor);
      float minFlingVelocity =
          mContext.getResources().getDimension(R.dimen.quickstep_fling_min_velocity);
      if (Math.abs(endVelocity) > minFlingVelocity && mTransitionDragLength > 0) {
        if (endTarget == RECENTS && mMode != Mode.NO_BUTTON) {
          Interpolators.OvershootParams overshoot =
              new Interpolators.OvershootParams(
                  startShift, endShift, endShift, velocityPxPerMs.y, mTransitionDragLength);
          endShift = overshoot.end;
          interpolator = overshoot.interpolator;
          duration =
              Utilities.boundToRange(
                  overshoot.duration, MIN_OVERSHOOT_DURATION, MAX_SWIPE_DURATION);
        } else {
  ...
  private void animateToProgress(
      ...
      GestureEndTarget target,
      PointF velocityPxPerMs) {
    mRecentsAnimationWrapper.runOnInit(
        () ->
            animateToProgressInternal(start, end, duration, interpolator, target, velocityPxPerMs));
  }
  ...
  private void animateToProgressInternal(
    ...
    if (mGestureEndTarget == HOME) {
      ...
      windowAnim.addAnimatorListener(
    ...
    TimeInterpolator adjustedInterpolator = Interpolators.mapToProgress(interpolator, start, end);
    if (mLauncherTransitionController == null) {
      return;
    }
  ...
  private RectFSpringAnim createWindowAnimationToHome(
      float startProgress, HomeAnimationFactory homeAnimationFactory) {
    final RemoteAnimationTargetSet targetSet = mRecentsAnimationWrapper.targetSet;
    final RectF startRect =
        new RectF(
            mClipAnimationHelper.applyTransform(
                targetSet, mTransformParams.setProgress(startProgress), false /* launcherOnTop */));
    ...
    anim.addOnUpdateListener(
        (currentRect, progress) -> {
          ...
          float windowAlpha =
              Utilities.boundToRange(
                  Utilities.mapToRange(alphaProgress, 0, windowAlphaThreshold, 1.5f, 0f, LINEAR),
                  0,
                  1);
          mTransformParams
          ...
          mClipAnimationHelper.applyTransform(
              targetSet, mTransformParams, false /* launcherOnTop */);
  ...
  private void startNewTask() {
    ...
    if (ENABLE_QUICKSTEP_LIVE_TILE) {
      ...
      mRecentsView
          .getTaskViewAt(mRecentsView.getNextPage())
          .launchTask(false /* animate */, true /* freezeTaskList */);
    } else {
      int taskId = mRecentsView.getTaskViewAt(mRecentsView.getNextPage()).getTask().key.id;
      mFinishingRecentsAnimationForNewTaskId = taskId;
      mRecentsAnimationWrapper.finish(
          true /* toRecents */,
          () -> {
            if (!mCanceled) {
  ...
  /**
   * Cancels any running animation so that the active target can be overriden by a new swipe handle
   * (in case of quick switch).
   */
  public void cancelCurrentAnimation(SwipeSharedState sharedState) {
    ...
    if (mFinishingRecentsAnimationForNewTaskId != -1) {
      ...
      switchToScreenshot();
      TaskView newRunningTaskView =
          mRecentsView.getTaskView(mFinishingRecentsAnimationForNewTaskId);
      int newRunningTaskId = newRunningTaskView != null ? newRunningTaskView.getTask().key.id : -1;
  ...
  private void finishCurrentTransitionToRecents() {
    if (ENABLE_QUICKSTEP_LIVE_TILE) {
      ...
      synchronized (mRecentsAnimationWrapper) {
        mRecentsAnimationWrapper.finish(
            true /* toRecents */, () -> setStateOnUiThread(STATE_CURRENT_TASK_FINISHED));
      }
    }
  }
  ...
  private static boolean isNotInRecents(RemoteAnimationTargetCompat app) {
    return app.isNotInRecents || app.activityType == RemoteAnimationTargetCompat.ACTIVITY_TYPE_HOME;
  }
