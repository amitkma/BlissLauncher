      ...
      case ACTION_MOVE:
        {
          ...
          if (!mPassedSlop) {
            ...
            if (Utilities.squaredHypot(mLastPos.x - mDownPos.x, mLastPos.y - mDownPos.y)
                > mSquaredSlop) {
              ...
              if (isValidAssistantGestureAngle(mDownPos.x - mLastPos.x, mDownPos.y - mLastPos.y)) {
                setActive(ev);
              } else {
                mState = STATE_DELEGATE_ACTIVE;
              }
            ...
            // Movement
            mDistance =
                (float) Math.hypot(mLastPos.x - mStartDragPos.x, mLastPos.y - mStartDragPos.y);
            if (mDistance >= 0) {
      ...
      case ACTION_UP:
        if (mState != STATE_DELEGATE_ACTIVE && !mLaunchedAssistant) {
          ValueAnimator animator =
              ValueAnimator.ofFloat(mLastProgress, 0).setDuration(RETRACT_ANIMATION_DURATION_MS);
