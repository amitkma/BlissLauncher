...
import foundation.e.blisslauncher.quickstep.TaskIconCache;
import foundation.e.blisslauncher.quickstep.TaskThumbnailCache;
import foundation.e.blisslauncher.quickstep.util.TaskCornerRadius;
...
public class TaskView extends FrameLayout implements RecentsView.PageCallbacks, ViewPool.Reusable {
  ...
  /**
   * The alpha of a black scrim on a page in the carousel as it leaves the screen. In the resting
   * position of the carousel, the adjacent pages have about half this scrim.
   */
  ...
  public AnimatorPlaybackController createLaunchAnimationForRunningTask() {
    ...
    AnimatorPlaybackController currentAnimation =
        AnimatorPlaybackController.wrap(pendingAnimation.anim, RECENTS_LAUNCH_DURATION, null);
  ...
  private void launchTaskInternal(
      boolean animate,
      boolean freezeTaskList,
      Consumer<Boolean> resultCallback,
      Handler resultCallbackHandler) {
    if (mTask != null) {
      ...
      if (animate) {
        ...
        ActivityManagerWrapper.getInstance()
            .startActivityFromRecentsAsync(mTask.key, opts, resultCallback, resultCallbackHandler);
      } else {
        opts =
  ...
  public void onTaskListVisibilityChanged(boolean visible) {
    ...
    if (visible) {
      ...
      mThumbnailLoadRequest =
          thumbnailCache.updateThumbnailInBackground(
      ...
      mIconLoadRequest =
          iconCache.updateIconInBackground(
              ...
              (task) -> {
                setIcon(task.icon);
                if (ENABLE_QUICKSTEP_LIVE_TILE && isRunningTask()) {
  ...
  private void setIconAndDimTransitionProgress(float progress, boolean invert) {
    ...
    float scale =
        Interpolators.clampToProgress(FAST_OUT_SLOW_IN, lowerClamp, upperClamp)
  ...
  public void animateIconScaleAndDimIntoView() {
    ...
    mIconAndDimAnimator.addListener(
  ...
  private class FooterWrapper extends ViewOutlineProvider {
    ...
    public FooterWrapper(View view) {
      ...
      mDelegate =
          mOldOutlineProvider == null ? ViewOutlineProvider.BACKGROUND : mOldOutlineProvider;
  ...
  public void setFullscreenProgress(float progress) {
    ...
    mCurrentFullscreenParams.setInsets(
        currentInsetsLeft,
    ...
    mCurrentFullscreenParams.setCornerRadius(
        Utilities.mapRange(mFullscreenProgress, mCornerRadius, fullscreenCornerRadius)
            / getRecentsView().getScaleX());
    // We scaled the thumbnail to fit the content (excluding insets) within task view width.
    // Now that we are drawing left/right insets again, we need to scale down to fit them.
    if (getWidth() > 0) {
      mCurrentFullscreenParams.setScale(
          getWidth() / (getWidth() + currentInsetsLeft + currentInsetsRight));
    }
