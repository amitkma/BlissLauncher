...
public class FlingAndHoldTouchController extends PortraitStatesTouchController {
    ...
    if (handlingOverviewAnim()) {
      mMotionPauseDetector.setOnMotionPauseListener(
          isPaused -> {
            ...
            mPeekAnim.start();
            recentsView.performHapticFeedback(
  ...
  /**
   * @return Whether we are handling the overview animation, rather than having it as part of the
   *     existing animation to the target state.
   */
  ...
  public void onDragEnd(float velocity, boolean fling) {
    if (mMotionPauseDetector.isPaused() && handlingOverviewAnim()) {
      ...
      AnimatorSet overviewAnim =
          mLauncher
              .getStateManager()
              .createAtomicAnimation(NORMAL, OVERVIEW, builder, ANIM_ALL, ATOMIC_DURATION);
      overviewAnim.addListener(
          new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
