...
public abstract class TaskViewTouchController<T extends BaseDraggingActivity>
    ...
    if (ev.getAction() == MotionEvent.ACTION_DOWN) {
      ...
      if (mCurrentAnimation != null) {
        ...
        for (int i = 0; i < mRecentsView.getTaskViewCount(); i++) {
          ...
          if (mRecentsView.isTaskViewVisible(view)
              && mActivity.getDragLayer().isEventOverView(view, ev)) {
  ...
  private void reInitAnimationController(boolean goingUp) {
    ...
    if (goingUp) {
      mPendingAnimation =
          mRecentsView.createTaskDismissAnimation(
              mTaskBeingDragged, true /* animateTaskView */, true /* removeTask */, maxDuration);
    ...
    mCurrentAnimation =
        AnimatorPlaybackController.wrap(mPendingAnimation.anim, maxDuration, this::clearState);
  ...
  public boolean onDrag(float displacement) {
    ...
    boolean isGoingUp = totalDisplacement == 0 ? mCurrentAnimationIsGoingUp : totalDisplacement < 0;
  ...
  public void onDragEnd(float velocity, boolean fling) {
    ...
    long animationDuration =
        SwipeDetector.calculateDuration(velocity, goingToEnd ? (1 - progress) : progress);
    if (blockedFling && !goingToEnd) {
      animationDuration *= LauncherAnimUtils.blockedFlingDurationFactor(velocity);
    }

    float nextFrameProgress =
