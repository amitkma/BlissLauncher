...
public class BackgroundAppState extends OverviewState {
  ...
  public LauncherState.ScaleAndTranslation getOverviewScaleAndTranslation(TestActivity launcher) {
    ...
    TaskView dummyTask =
        recentsView.getTaskViewAt(Math.max(taskCount - 1, recentsView.getCurrentPage()));
    return recentsView
        .getTempClipAnimationHelper()
  ...
  public ScaleAndTranslation getHotseatScaleAndTranslation(TestActivity launcher) {
    if ((getVisibleElements(launcher) & HOTSEAT_ICONS) != 0) {
      ...
      scaleAndTranslation.translationY =
          LayoutUtils.getShelfTrackingDistance(launcher, launcher.getDeviceProfile());
