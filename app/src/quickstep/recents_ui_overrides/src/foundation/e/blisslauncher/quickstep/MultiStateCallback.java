...
public class MultiStateCallback {
    ...
    if (DEBUG_STATES) {
      Log.d(
          ...
          "["
              ...
              + "] Adding "
              + convertToFlagNames(stateFlag)
              + " to "
              + convertToFlagNames(mState));
    }
  ...
  /**
   * Adds the provided state flags to the global state and executes any change handlers as a result.
   */
  ...
  /**
   * Sets the callbacks to be run when the provided states are enabled. The callback is only run
   * once.
   */
  public void addCallback(int stateMask, Runnable callback) {
    // TODO: gracefully ignore in production release.
    if (mCallbacks.get(stateMask) != null) {
