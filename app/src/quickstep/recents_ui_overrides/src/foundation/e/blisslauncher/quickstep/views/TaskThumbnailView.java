...
import foundation.e.blisslauncher.quickstep.TaskOverlayFactory.TaskOverlay;
...
public class TaskThumbnailView extends View {
   ...
   *
   * <p>If dimAlpha is 0, no dimming is applied; if dimAlpha is 1, the thumbnail will be black.
   */
  ...
  public int getSysUiStatusNavFlags() {
    if (mThumbnailData != null) {
      int flags = 0;
      flags |=
          (mThumbnailData.systemUiVisibility & SYSTEM_UI_FLAG_LIGHT_STATUS_BAR) != 0
              ? SystemUiController.FLAG_LIGHT_STATUS
              : SystemUiController.FLAG_DARK_STATUS;
      flags |=
          (mThumbnailData.systemUiVisibility & SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR) != 0
  ...
  public void drawOnCanvas(
      Canvas canvas, float x, float y, float width, float height, float cornerRadius) {
    if (ENABLE_QUICKSTEP_LIVE_TILE) {
      if (mTask != null && getTaskView().isRunningTask() && !getTaskView().showScreenshot()) {
        canvas.drawRoundRect(x, y, width, height, cornerRadius, cornerRadius, mClearPaint);
        canvas.drawRoundRect(
            x, y, width, height, cornerRadius, cornerRadius, mDimmingPaintAfterClearing);
        return;
      }
    }
    ...
    final boolean drawBackgroundOnly =
        mTask == null || mTask.isLocked || mBitmapShader == null || mThumbnailData == null;
  ...
  private void updateThumbnailMatrix() {
    ...
    if (mBitmapShader != null && mThumbnailData != null) {
      ...
      final float thumbnailWidth =
          mThumbnailData.thumbnail.getWidth()
      ...
      if (getMeasuredWidth() == 0) {
        ...
        thumbnailScale =
            isRotated ? getMeasuredWidth() / thumbnailHeight : getMeasuredWidth() / thumbnailWidth;
      }
      ...
      float bitmapHeight =
          Math.max((isRotated ? thumbnailWidth : thumbnailHeight) * thumbnailScale, 0);
  ...
  private static ColorFilter getColorFilter(
      int intensity, boolean shouldLighten, float saturation) {
