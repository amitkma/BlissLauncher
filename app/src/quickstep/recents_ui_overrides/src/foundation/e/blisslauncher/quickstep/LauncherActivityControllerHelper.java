...
import static foundation.e.blisslauncher.features.test.LauncherState.BACKGROUND_APP;
...
import static foundation.e.blisslauncher.features.test.anim.Interpolators.ACCEL_DEACCEL;
...
public final class LauncherActivityControllerHelper implements ActivityControlHelper<TestActivity> {
  ...
  public int getSwipeUpDestinationAndLength(
      VariantDeviceProfile dp, Context context, Rect outRect) {
  ...
  public HomeAnimationFactory prepareHomeUI(TestActivity activity) {
    ...
    return new HomeAnimationFactory() {
      ...
      public RectF getWindowTargetRect() {
        final int halfIconSize = dp.getIconSizePx() / 2;
        final float targetCenterX = dp.getAvailableWidthPx() / 2f;
        final float targetCenterY = dp.getAvailableHeightPx() - dp.getHotseatBarSizePx();
        ...
        if (canUseWorkspaceView) {
          ...
          return new RectF(
              targetCenterX - halfIconSize,
              targetCenterY - halfIconSize,
              targetCenterX + halfIconSize,
              targetCenterY + halfIconSize);
        }
      }
      ...
      public AnimatorPlaybackController createActivityAnimationToHome() {
        ...
        return activity
            .getStateManager()
            .createAnimationToNewWorkspace(NORMAL, accuracy, 0 /* animComponents */);
      }
  ...
  public AnimationFactory prepareRecentsUI(
    ...
    return new AnimationFactory() {
      ...
      public void setShelfState(
          ShelfAnimState shelfState, Interpolator interpolator, long duration) {
        ...
        activity
            .getStateManager()
            .cancelStateElementAnimation(LauncherAppTransitionManagerImpl.INDEX_SHELF_ANIM);
        ...
        float shelfPeekingProgress =
            shelfHiddenProgress - (shelfHiddenProgress - defaultOverviewProgress) * 0.25f;
        float toProgress =
            mShelfState == ShelfAnimState.HIDE
                ? shelfHiddenProgress
                : mShelfState == ShelfAnimState.PEEK ? shelfPeekingProgress : shelfOverviewProgress;
        Animator shelfAnim =
            activity
                .getStateManager()
                .createStateElementAnimation(
                    LauncherAppTransitionManagerImpl.INDEX_SHELF_ANIM, toProgress);
      ...
      public void setRecentsAttachedToAppWindow(boolean attached, boolean animate) {
        ...
        fadeAnim
            .setDuration(animate ? WindowTransformSwipeHandler.RECENTS_ATTACH_DURATION : 0)
            .start();
      }
    };
  }

  private void createActivityControllerInternal(
      TestActivity activity,
    ...
    controller.setEndAction(
        () -> {
          activity
              .getStateManager()
              .goToState(controller.getInterpolatedProgress() > 0.5 ? endState : fromState, false);
        });
    callback.accept(controller);
  }

  /** Scale down recents from the center task being full screen to being in overview. */
  private void playScaleDownAnim(
      AnimatorSet anim, TestActivity launcher, LauncherState fromState, LauncherState endState) {
    ...
    Animator scale =
        ObjectAnimator.ofFloat(
            ...
            endScaleAndTranslation.scale);
    Animator translateY =
        ObjectAnimator.ofFloat(recentsView, TRANSLATION_Y, fromTranslationY, endTranslationY);
    Animator applyFullscreenProgress =
        ObjectAnimator.ofFloat(
            recentsView,
            RecentsView.FULLSCREEN_PROGRESS,
            fromFullscreenProgress,
            endFullscreenProgress);
  ...
  public boolean switchToRecentsIfVisible(Runnable onCompleteCallback) {
    ...
    launcher
        .getStateManager()
        .goToState(
            OVERVIEW, launcher.getStateManager().shouldAnimateStateChange(), onCompleteCallback);
    return true;
  }
