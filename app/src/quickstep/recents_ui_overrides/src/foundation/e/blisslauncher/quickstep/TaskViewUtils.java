...
public final class TaskViewUtils {
   ...
   *
   * <p>If this method returns a non-null TaskView, it will be used in
   * composeRecentsLaunchAnimation. Otherwise, we will assume we are using a normal app transition,
   * but it's possible that the opening remote target (which we don't get until onAnimationStart)
   * will resolve to a TaskView.
   */
   ...
   *     animation.
   */
  public static ValueAnimator getRecentsWindowAnimator(
      TaskView v,
      boolean skipViewChanges,
      RemoteAnimationTargetCompat[] targets,
      final ClipAnimationHelper inOutHelper) {
    SyncRtSurfaceTransactionApplierCompat applier = new SyncRtSurfaceTransactionApplierCompat(v);
    ClipAnimationHelper.TransformParams params =
        new ClipAnimationHelper.TransformParams().setSyncTransactionApplier(applier);
        ...
        new MultiValueUpdateListener() {
...

          {
            ...
            inOutHelper.prepareAnimation(
                BaseActivity.fromContext(v.getContext()).getDeviceProfile(), true /* isOpening */);
            inOutHelper.fromTaskThumbnailView(
                v.getThumbnail(),
                (RecentsView) v.getParent(),
