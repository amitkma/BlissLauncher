...
import static foundation.e.blisslauncher.features.test.BaseActivity.INVISIBLE_ALL;
import static foundation.e.blisslauncher.features.test.BaseActivity.INVISIBLE_BY_APP_TRANSITIONS;
...
import static foundation.e.blisslauncher.features.test.anim.Interpolators.EXAGGERATED_EASE;
import static foundation.e.blisslauncher.features.test.anim.Interpolators.LINEAR;
import static foundation.e.blisslauncher.features.test.dragndrop.DragLayer.ALPHA_INDEX_TRANSITIONS;
...
import foundation.e.blisslauncher.features.test.LauncherAppTransitionManager;
...
import foundation.e.blisslauncher.quickstep.util.MultiValueUpdateListener;
...
public abstract class QuickstepAppTransitionManagerImpl extends LauncherAppTransitionManager
  ...
  public ActivityOptions getActivityLaunchOptions(TestActivity launcher, View v) {
    if (hasControlRemoteAppTransitionPermission()) {
      ...
      RemoteAnimationRunnerCompat runner =
          new LauncherAnimationRunner(mHandler, true /* startAtFrontOfQueue */) {
  ...
  /**
   * Whether the launch is a recents app transition and we should do a launch animation from the
   * recents view. Note that if the remote animation targets are not provided, this may not always
   * be correct as we may resolve the opening app to a task when the animation starts.
   *
   ...
   */
  protected abstract boolean isLaunchingFromRecents(
      @NonNull View v, @Nullable RemoteAnimationTargetCompat[] targets);
   ...
   */
  protected abstract void composeRecentsLaunchAnimator(
  ...
  private void composeIconLaunchAnimator(
    ...
    anim.play(getOpeningWindowAnimators(v, targets, windowTargetBounds, !isAllOpeningTargetTrs));
    if (launcherClosing) {
      Pair<AnimatorSet, Runnable> launcherContentAnimator =
          getLauncherContentAnimator(true /* isAppOpening */, new float[] {0, -mContentTransY});
  ...
  /**
   * Return the window bounds of the opening target. In multiwindow mode, we need to get the final
   * size of the opening app window target to help figure out where the floating view should animate
   * to.
   */
  private Rect getWindowTargetBounds(RemoteAnimationTargetCompat[] targets) {
    Rect bounds = new Rect(0, 0, mDeviceProfile.getWidthPx(), mDeviceProfile.getHeightPx());
  ...
  public void setRemoteAnimationProvider(
      final RemoteAnimationProvider animationProvider, CancellationSignal cancellationSignal) {
   ...
   *
   * @param isAppOpening True when this is called when an app is opening. False when this is called
   *     when an app is closing.
   * @param trans Array that contains the start and end translation values for the content.
   */
  private Pair<AnimatorSet, Runnable> getLauncherContentAnimator(
      boolean isAppOpening, float[] trans) {
    ...
    // TODO: DO it later.
    // mLauncher.getLauncherPagedView().getPageIndicator().pauseAnimations();
   ...
   */
  protected abstract Runnable composeViewContentAnimator(
      @NonNull AnimatorSet anim, float[] alphas, float[] trans);

  /** @return Animator that controls the window of the opening targets. */
  private ValueAnimator getOpeningWindowAnimators(
      View v,
    ...
    if (v instanceof IconTextView) {
      bounds.set(((IconTextView) v).getIconBounds());
    } else {
    ...
    float startScale = 1f;
    /*if (v instanceof IconTextView && !(v.getParent() instanceof DeepShortcutView)) {
        Drawable dr = ((IconTextView) v).getIcon();
    ...
    boolean useUpwardAnimation =
        bounds.top > centerY || Math.abs(dY) < mLauncher.getDeviceProfile().getCellHeightPx();
    final long xDuration =
        useUpwardAnimation ? APP_LAUNCH_CURVED_DURATION : APP_LAUNCH_DOWN_DURATION;
    final long yDuration =
        useUpwardAnimation ? APP_LAUNCH_DURATION : APP_LAUNCH_DOWN_CURVED_DURATION;
    final long alphaDuration =
        useUpwardAnimation ? APP_LAUNCH_ALPHA_DURATION : APP_LAUNCH_ALPHA_DOWN_DURATION;
    ...
    appAnimator.addListener(
        new AnimatorListenerAdapter() {
          ...
          public void onAnimationEnd(Animator animation) {
            if (v instanceof IconTextView) {
              ((IconTextView) v).setStayPressed(false);
            }
            openingTargets.release();
          }
        });

    float shapeRevealDuration = APP_LAUNCH_DURATION;

    final float startCrop;
    final float endCrop;
    // TODO: May enable when supporting landscape
    ...
    final float initialWindowRadius =
        supportsRoundedCornersOnWindows(mLauncher.getResources()) ? startCrop / 2f : 0f;
    final float windowRadius =
        mDeviceProfile.isMultiWindowMode ? 0 : getWindowCornerRadius(mLauncher.getResources());
    appAnimator.addUpdateListener(
        new MultiValueUpdateListener() {
          FloatProp mDx = new FloatProp(0, dX, 0, xDuration, AGGRESSIVE_EASE);
          FloatProp mDy = new FloatProp(0, dY, 0, yDuration, AGGRESSIVE_EASE);
          FloatProp mIconScale =
              new FloatProp(initialStartScale, scale, 0, APP_LAUNCH_DURATION, EXAGGERATED_EASE);
          FloatProp mIconAlpha =
              new FloatProp(1f, 0f, APP_LAUNCH_ALPHA_START_DELAY, alphaDuration, LINEAR);
          FloatProp mCroppedSize =
              new FloatProp(startCrop, endCrop, 0, CROP_DURATION, EXAGGERATED_EASE);
          FloatProp mWindowRadius =
              new FloatProp(
                  initialWindowRadius, windowRadius, 0, RADIUS_DURATION, EXAGGERATED_EASE);
  ...
  private void registerRemoteAnimations() {
    ...
    if (hasControlRemoteAppTransitionPermission()) {
      ...
      definition.addRemoteAnimation(
          WindowManagerWrapper.TRANSIT_WALLPAPER_OPEN,
          WindowManagerWrapper.ACTIVITY_TYPE_STANDARD,
          new RemoteAnimationAdapterCompat(
              getWallpaperOpenRunner(false /* fromUnlock */),
  ...
  /**
   * @return Runner that plays when user goes to Launcher ie. pressing home, swiping up from nav
   *     bar.
   */
  RemoteAnimationRunnerCompat getWallpaperOpenRunner(boolean fromUnlock) {
    return new WallpaperOpenLauncherAnimationRunner(
        mHandler, false /* startAtFrontOfQueue */, fromUnlock);
  }
  ...
  private Animator getClosingWindowAnimators(RemoteAnimationTargetCompat[] targets) {
    ...
    closingAnimator.addUpdateListener(
        new MultiValueUpdateListener() {
  ...
  private void createLauncherResumeAnimation(AnimatorSet anim) {
    ...
    ;
    workspaceAnimator.play(
        ObjectAnimator.ofFloat(mDragLayer, View.TRANSLATION_Y, -mWorkspaceTransY, 0));

    mDragLayerAlpha.setValue(0);
    workspaceAnimator.play(ObjectAnimator.ofFloat(mDragLayerAlpha, MultiValueAlpha.VALUE, 0, 1f));

    workspaceAnimator.setStartDelay(LAUNCHER_RESUME_START_DELAY);
    workspaceAnimator.setDuration(333);
    workspaceAnimator.setInterpolator(DEACCEL_1_7);

    // Pause page indicator animations as they lead to layer trashing.
    // mLauncher.getLauncherPagedView().getPageIndicator().pauseAnimations();
    mDragLayer.setLayerType(View.LAYER_TYPE_HARDWARE, null);

    workspaceAnimator.addListener(
        new AnimatorListenerAdapter() {
          @Override
          public void onAnimationEnd(Animator animation) {
            resetContentView();
  ...
  private void resetContentView() {
    // mLauncher.getLauncherPagedView().getPageIndicator().skipAnimationsToEnd();
  ...
  class WallpaperOpenLauncherAnimationRunner extends LauncherAnimationRunner {
    ...
    public WallpaperOpenLauncherAnimationRunner(
        Handler handler, boolean startAtFrontOfQueue, boolean fromUnlock) {
          ...
          if (mFromUnlock) {
            Pair<AnimatorSet, Runnable> contentAnimator =
                getLauncherContentAnimator(
                    false /* isAppOpening */, new float[] {mContentTransY, 0});
