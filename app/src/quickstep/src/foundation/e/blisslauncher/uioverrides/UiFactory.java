...
public class UiFactory extends RecentsUiFactory {
  ...
  public static Runnable enableLiveUIChanges(TestActivity launcher) {
    SysUINavigationMode.NavigationModeChangeListener listener =
        m -> {
          launcher.getDragLayer().recreateControllers();
          launcher
              .getRotationHelper()
              .setRotationHadDifferentUI(m != SysUINavigationMode.Mode.NO_BUTTON);
        };
  ...
  public static void onLauncherStateOrFocusChanged(TestActivity launcher) {
    boolean shouldBackButtonBeHidden =
        launcher != null
    ...
    OverviewInteractionState.INSTANCE
        .get(launcher)
  ...
  public static void useFadeOutAnimationForLauncherStart(
      TestActivity launcher, CancellationSignal cancellationSignal) {
  ...
  public static boolean dumpActivity(Activity activity, PrintWriter writer) {
    ...
    writer.println(Base64.encodeToString(out.toByteArray(), Base64.NO_WRAP | Base64.NO_PADDING));
    return true;
  }
   ...
   *
   * <p>ProxyActivityStarter is started with clear task to reset the task after which it removes the
   * task itself.
   */
  ...
  public static LauncherState.ScaleAndTranslation getOverviewScaleAndTranslationForNormalState(
      TestActivity l) {
    if (SysUINavigationMode.getMode(l) == SysUINavigationMode.Mode.NO_BUTTON) {
      float offscreenTranslationX =
          l.getDeviceProfile().getWidthPx() - l.getOverviewPanel().getPaddingStart();
