...
public class PortraitStatesTouchController extends AbstractStateChangeTouchController {
  ...
  protected float initCurrentAnimation(
    ...
    if (mFromState == OVERVIEW
      ...
      mPendingAnimation =
          mOverviewPortraitStateTouchHelper.createSwipeDownToTaskAppAnimation(maxAccuracy);
      ...
      mCurrentAnimation =
          AnimatorPlaybackController.wrap(mPendingAnimation.anim, maxAccuracy, onCancelRunnable);
      ...
      mCurrentAnimation =
          mLauncher
              ...
              .createAnimationToNewWorkspace(
                  mToState, builder, maxAccuracy, this::clearState, animComponents);
    }

    if (totalShift == 0) {
      totalShift =
          Math.signum(mFromState.ordinal - mToState.ordinal)
  ...
  private void handleFirstSwipeToOverview(
    ...
    if (mFromState == NORMAL && mToState == OVERVIEW && targetState == OVERVIEW) {
      ...
      if (isFling && expectedDuration != 0) {
        ...
        animator.setDuration(Math.min(expectedDuration, ATOMIC_DURATION)).setInterpolator(LINEAR);
      }
    } else {
