...
public abstract class BaseRecentsViewStateController<T extends View>
  ...
  public void setState(@NonNull LauncherState state) {
    LauncherState.ScaleAndTranslation scaleAndTranslation =
        state.getOverviewScaleAndTranslation(mLauncher);
  ...
  void setStateWithAnimationInternal(
    ...
    LauncherState.ScaleAndTranslation scaleAndTranslation =
        toState.getOverviewScaleAndTranslation(mLauncher);
    ...
    Interpolator translateXInterpolator =
        builder.getInterpolator(ANIM_OVERVIEW_TRANSLATE_X, LINEAR);
    Interpolator translateYInterpolator =
        builder.getInterpolator(ANIM_OVERVIEW_TRANSLATE_Y, LINEAR);
    float translationX = scaleAndTranslation.translationX;
    if (mRecentsView.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL) {
      translationX = -translationX;
    }
    ...
    setter.setFloat(
        mRecentsView, View.TRANSLATION_Y, scaleAndTranslation.translationY, translateYInterpolator);
    setter.setFloat(
        mRecentsView,
        getContentAlphaProperty(),
        toState.overviewUi ? 1 : 0,
