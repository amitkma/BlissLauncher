...
public class LayoutUtils {
  ...
  public static void calculateLauncherTaskSize(
      Context context, VariantDeviceProfile dp, Rect outRect) {
    float extraSpace;
    extraSpace = dp.getHotseatBarSizePx() + dp.getVerticalDragHandleSizePx();
    calculateTaskSize(context, dp, extraSpace, MULTI_WINDOW_STRATEGY_HALF_SCREEN, outRect);
  }

  public static void calculateFallbackTaskSize(
      Context context, VariantDeviceProfile dp, Rect outRect) {
    calculateTaskSize(context, dp, 0, MULTI_WINDOW_STRATEGY_DEVICE_PROFILE, outRect);
  }

  @AnyThread
  public static void calculateTaskSize(
      Context context,
      VariantDeviceProfile dp,
      float extraVerticalSpace,
      @MultiWindowStrategy int multiWindowStrategy,
      Rect outRect) {
    ...
    float availableHeight =
        launcherVisibleHeight - topIconMargin - extraVerticalSpace - paddingVert;
    ...
    float y =
        insets.top
            + Math.max(topIconMargin, (launcherVisibleHeight - extraVerticalSpace - outHeight) / 2);
    outRect.set(
        Math.round(x),
        Math.round(y),
        Math.round(x) + Math.round(outWidth),
        Math.round(y) + Math.round(outHeight));
  }

  public static int getShelfTrackingDistance(Context context, VariantDeviceProfile dp) {
    // Track the bottom of the window.
    int shelfHeight = dp.getHotseatBarSizePx() + dp.getInsets().bottom;
    int spaceBetweenShelfAndRecents =
        (int) context.getResources().getDimension(R.dimen.task_card_vert_space);
