...
public class NormalizedIconLoader extends IconLoader {
  ...
  public Drawable getDefaultIcon(int userId) {
    synchronized (mDefaultIcons) {
      ...
      if (info == null) {
        info =
            getBitmapInfo(
                Resources.getSystem().getDrawable(android.R.drawable.sym_def_app_icon), userId);
  ...
  @Override
  protected Drawable getBadgedActivityIcon(
      ActivityInfo activityInfo, int userId, TaskDescription desc) {
    return getBitmapInfo(activityInfo.loadUnbadgedIcon(mContext.getPackageManager()), userId);
  }
}
