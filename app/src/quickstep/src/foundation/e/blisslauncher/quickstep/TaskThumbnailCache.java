...
public class TaskThumbnailCache {
  ...
  public void updateThumbnailInCache(Task task) {
    ...
    if (task.thumbnail == null) {
      updateThumbnailInBackground(task.key, true /* reducedResolution */, t -> task.thumbnail = t);
    }
  }
  ...
  public ThumbnailLoadRequest updateThumbnailInBackground(
    ...
    return updateThumbnailInBackground(
        task.key,
        !mHighResLoadingState.isEnabled(),
        t -> {
          task.thumbnail = t;
          callback.accept(t);
        });
  ...
  private ThumbnailLoadRequest updateThumbnailInBackground(
      TaskKey key, boolean reducedResolution, Consumer<ThumbnailData> callback) {
    Preconditions.assertUIThread();
    ...
    ThumbnailLoadRequest request =
        new ThumbnailLoadRequest(mBackgroundHandler, reducedResolution) {
          @Override
          public void run() {
            ThumbnailData thumbnail =
                ActivityManagerWrapper.getInstance().getTaskThumbnail(key.id, reducedResolution);
            if (isCanceled()) {
  ...
  public abstract static class ThumbnailLoadRequest extends HandlerRunnable {
