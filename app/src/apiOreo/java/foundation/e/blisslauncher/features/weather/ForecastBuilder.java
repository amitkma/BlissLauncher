...
public class ForecastBuilder {
  ...
  public static void buildLargePanel(Context context, View weatherPanel, WeatherInfo w) {
    ...
    if (windSpeedUnit == MPH && useMetric) {
      windSpeedUnit = KPH;
      windSpeed =
          foundation.e.blisslauncher.features.weather.WeatherUtils.milesToKilometers(windSpeed);
    } else if (windSpeedUnit == KPH && !useMetric) {
      windSpeedUnit = MPH;
      windSpeed =
          foundation.e.blisslauncher.features.weather.WeatherUtils.kilometersToMiles(windSpeed);
    }
    ...
    weatherHumWind.setText(
        String.format(
            ...
            foundation.e.blisslauncher.features.weather.WeatherUtils.resolveWindDirection(
  ...
  private static void buildSmallPanel(Context context, LinearLayout smallPanel, WeatherInfo w) {
    ...
    int itemSidePadding =
        context.getResources().getDimensionPixelSize(R.dimen.forecast_item_padding_side);
    ...
    for (int count = 0; count < numForecasts; count++) {
      ...
      String iconsSet = Preferences.getWeatherIconSet(context);
      final int resId =
          WeatherIconUtils.getWeatherIconResource(context, iconsSet, d.getConditionCode());
      if (resId != 0) {
    ...
    smallPanel.setOnClickListener(
        v -> {
          Intent launchIntent =
              context.getPackageManager().getLaunchIntentForPackage("foundation.e.weather");
